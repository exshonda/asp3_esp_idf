#!/usr/bin/env bash
#
#  ESP32-C6 BLE（ble_host_smoke_c6）を «Espressif版 riscv32-esp-elf GCC 15.2.0»
#  でビルド→flash→boot する実機ヘルパ（c6ble.sh の esp-15.2 版）。
#
#  c6ble.sh（既定＝xpack riscv-none-elf-gcc 15.2.0）との差分は以下の3点のみ
#  （c5ble_esp15.sh/c3ble_esp15.shと同じパターン）：
#   - GCC_BIN を esp-15.2 の bin へ
#   - -DRISCV64_TOOLCHAIN_PREFIX=riscv32-esp-elf-（PATHだけでは不十分。
#     cmake呼び出しに焼き込まれているprefixを明示的に上書きする）
#   - BUILD dir を別（build/c6ble_esp15）にして xpack ビルドと衝突させない
#
#  march/mabi は C6 chip.cmake 固定＝-march=rv32imc_zicsr_zifencei -mabi=ilp32。
#  esp-15.2 のmultilibに `rv32imc_zicsr_zifencei/ilp32` が厳密一致で存在
#  （C3/C5と同一．`riscv32-esp-elf-gcc -print-multi-lib` で確認済み）。
#
#  使い方: c6ble.sh と同じ（build/flash/boot/all/port）。既定app=ble_host_smoke_c6。
#
set -u

BOARD_MAC="${BOARD_MAC:?BOARD_MAC を指定してください（対象ボードの MAC）。公開時にスクラブしたため既定値は持ちません}"		# board C (port1)
...
BUILD="${C6BLE_BUILD:-$REPO/build/c6ble_esp15}"
...
do_build() {
	...
	echo "## configure ($app) -> $BUILD [esp-15.2, prefix=$TOOLCHAIN_PREFIX] opts=${opts[*]} $*"
	rm -rf "$BUILD"
