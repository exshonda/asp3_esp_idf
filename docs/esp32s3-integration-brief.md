...
2. **可逆・小さく**：cmake のパス定義で切替わる粒度を保ち、C3/C5/C6 の既存ビルドを
   ...
   **submodule レイアウト・シム規約（FreeRTOS 互換/sdkconfig/cmake）・ブートハンドオフは
   「C 系（C3/C5/C6）の IDF 移行の共通土台」として設計する**——S3 がこのパターンを先行
   実証し、後で C 系が同じ土台へ後追い移行する前提（decision メモ §7）。S3 固有に閉じた
   作りにせず、RISC-V/Xtensa で共有できる部分（IDF コンポーネント引き込み・WiFi/BT シム）
   と、アーキ固有部（linker/startup・カーネルポート）を分離しておくこと。
