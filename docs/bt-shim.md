...
### ★広告非到達 デバイス側判別ラウンド（2026-07-15, board B 60:55:F9:57:C2:60 @ttyACM3）：Direct Boot 健全・**adv rc=0 到達**を無条件マーカ＋sentinel で確定＝フェーズA「SYNC未到達」は board 状態依存の一過性
...
**症状**：C3 D-2c/D-2d フル達成（0xABF1〜4 OTA・上記）直後、**C3(`ASP3-C3-BLE`/board B `60:55:F9:57:C2:60`)＋C6(`ASP3-C6-BLE`/board C `14:C1:9F:E0:5A:9C`)の両方が、Android/iPhone の両方から不可視**になった。数分前は同じC3が同じスマホで見えて BT4-OK まで読めていた。
