...
rev v0.3，MAC `<MAC-21>`）に対する実機JTAGで真因を特定した。
...
- **OpenOCD版上げ完了**：`openocd-esp32 v0.12.0-esp32-20260703`
  （`$HOME/tools/espressif/tools/openocd-esp32/...`）。旧dev版
...
- 既存NuttXツリー（C6調査流用）をコピーし `esp32c3-devkit:ble`
  ...
  基準機A（`<MAC-23>`）へ書込み，NuttXがboot時にem_base_init
...
リセットするかをbp二分で局所化」を被験機B（`<MAC-21>`）で実施。
...
**実機検証（被験機B `<MAC-21>`）**＝**★enable完走・VHCI往復成功
＝Phase D-1完了基準を達成**：
...
serial `<MAC-23>`・port 13333系・board B `...C2:60`と混同厳禁；S3テストで
...
board A（serial `<MAC-23>`）へ D-1で使ったNuttX-C3-BLE
...
`<MAC-24> "NuttX"` を強RSSI(-39〜-41)で連続検出**＝board Aの
...
SWとrevの交絡だった．→ **board B(rev3)にNuttXを焼いて確認：`<MAC-22> "NuttX"`が
...
- **④電波**：ホストhci0の`bluetoothctl scan le`で`ASP3-C3-BLE`
  （`<MAC-21>`）を**2ブート＋終了処理後の再起動＝計3回独立に検出**．
...
### 症状の再現（現ツリー 802d75b）
ホスト hci0（Intel AX210，`<MAC-26>`）から
`bluetoothctl connect <MAC-21>` → `Connected=true` だが
...
特性）＋周期notify**を実装し，BlueZ hci0（Intel AX210 `<MAC-26>`）
...
### 検証結果（over-the-air，BlueZ hci0×board B `<MAC-21>`）
...
### 実機状態（クローズ）
board B（`<MAC-21>`）に **SM入り本番ビルドを書込み済み・advertising継続**
...
#### DUT側 RTC マーカ（`tmp/c3ble.sh mark`，board B `<MAC-21>`）
| reg | 値 | 意味 |
|---|---|---|
...
sm_bonding/sm_sc/key_dist ENC|ID）を esp32c3 でビルドし，board B（<MAC-21>，
BLE=`nimble-bleprph` <MAC-22>）へ焼いて実機テスト**。
...
`<MAC-37>`）。マーカは C5 の LP_AON STORE «0-9のみ実在» のため STORE6
...
#### HCIフロー制御仮説の反証（同日）：BLE_HS_FLOW_CTRL は実効=0（無効）＝非因果
「1個目OK・暗号後2個で停止」から controller→host ACL の credit 管理（BLE_HS_FLOW_CTRL＝
Host Buffer Size/Number of Completed Packets）を疑ったが，我々のビルドの実効値を
プリプロセッサに直接確認（`riscv-none-elf-gcc -E` で MYNEWT_VAL_BLE_HS_FLOW_CTRL）＝
**(0)＝無効**。C5 sdkconfig_stub にも NuttX C3(同blob) にも CONFIG_BT_NIMBLE_HS_FLOW_CTRL
定義なし。FLOW_CTRL 無効＝host は controller→host flow control を «有効化しない»＝
controller は ACL を credit 待ちせず自由送信＝「host側の完了通知(ACK)不達で controller が
2個目を止める」機序は成立しない．∴フロー制御は反証。残る核心＝FLOW_CTRL 無効で controller は
自由送信できるのに，暗号後の鍵配布 ACL が host の ble_sm に届かない＝トランスポート RX 経路
（controller→host_rcv_pkt→ble_transport_to_hs_acl→ble_mqueue_put→ble_sm_rx）の
«我々 vs IDF/S3» 差分を精査（read-only 比較調査へ）。

#### RX経路 read-only 比較調査の結論（同日）：真因は «コントローラ側» に確定、host ソフトは無罪。次の決定打＝host_rcv_pkt 計装で A/B 決着
general-purpose エージェントに controller→host ACL RX 経路の «我々 vs IDF/S3/NuttX»
比較を委譲（read-only）。結論：
- **RX ソースは IDF v6.1 とバイト同一**（esp_nimble_hci.c/transport.c/ble_hs.c/mqueue、
  ログマクロ差のみ）。host_rcv_pkt→ble_hci_rx_acl→ble_transport_to_hs_acl→
  ble_mqueue_put(STAILQに無条件挿入)→host task が while で全ドレイン。2個目を«STAILQに
  入った後»に失う経路は無い。
- **決定的交絡排除**：C3 は upstream hal npl、C5 は IDF v6.1 npl＝«異なる» eventq/queued
  実装。なのに両方同一失敗＝npl/eventq/mqueue 層は全て無罪。esp_shim の sem/queue 会計も
  収支ゼロ（free_top=depth/sem=depth/debt=0 に復帰）、critical section 配線も正常。
- **論理の要**：LL 暗号(CCM)はコントローラ内で完結＝host の HCI ACL は常に平文 L2CAP。
  host ソフトに「暗号化 ACL #2 を平文と違う扱いにする分岐」は存在し得ない。∴«暗号化かつ
  2個目以降»固有の失敗はコントローラの LL/CCM 経路にある。我々がコントローラに触れる4面
  （clock/power・os_adapter・INTMTX・config）のうち config/os_adapter/intr は反証済み。
  残る«C3/C5共通・IDF相違・per-packet»面＝**clock/power**（Direct Boot が
  esp_perip_clk_init/pmu_init を飛ばし最小 BB マスク 0x00031840 のみ＝bt/bt_shim.c:76,90。
  C5 MODEM ドメイン無給電と同族 [[c5-wifi-modem-domain-unpowered]]）。
- **clock 説の弱点（要反証）**：暗号後1個目 ACL は届き処理された（put=1）＝CCM 復号は #1 で
  成功。クロック不足なら #1 も失敗するはず。なぜ #1 は復号でき #2 は消えるか未説明＝生存仮説
  であって証明ではない。
- **測定ギャップ＝決定打**：過去の計装は全て ble_mqueue_put «以下»。誰も host_rcv_pkt
  （コントローラ→host の入口、blob が呼ぶ＝cross-TU で --wrap 可）を数えていない。「put=1」は
  (A)コントローラが2個目 host_rcv_pkt を呼ばない[controller/LL/CCM/clock] と
  (B)host_rcv_pkt は呼ばれたが ble_hci_rx_acl の alloc 失敗ループ(esp_nimble_hci.c:190-207
  retry_count>=30)で put 前に落ちた[host側] を区別できない。次＝host_rcv_pkt を --wrap し
  暗号マーカ以降の ACL(data[0]==0x02)呼数＋alloc-stuck giveup を RTC 記録＝A/B 決着。
  A なら bt_shim.c の BT_BB クロックマスクを modem/FE/PHY/AES へ拡張(我々側・禁則非該当)して
  再試験。B なら pool/pend 経路。

### ★★★ D-2d bond 真因 «確定»（同日）：BLE_HS_PVCY=0 が responder の Identity 鍵配布をコンパイルアウト（working S3 は PVCY=1）
C5 RXTRACE の双方向計装で真因を «完全に» 局在化・裏取りした。決定的マーカ（board 電源
再投入後のクリーン測定）：`enc_chg=1・sm_enc_rx=1・put=1・sm_tx=0・to_ll=1`，
SM=0x5de01e0d（ETIMEOUT・**delta=30s**）。読み：
1. HCI Encryption Change は host 到達（enc_chg=1）**かつ** ble_sm_enc_change_rx まで
   dispatch 到達（sm_enc_rx=1）＝dispatch/配送は正常。
2. delta=30s＝enc は «接続直後に早く» 届いた（30sで遅れて来たのではない）＝配送遅延を反証。
3. **なのに ble_sm_tx=0**＝我々の SM は暗号後30秒間 SMP PDU を1つも送っていない。
   NimBLE 規則（ble_sm.c:1291「responder は鍵があれば先に送る」，我々=responder→
   res.execute=1）では sm_tx>0 のはず。

真因＝**ble_sm_key_exch_exec の Identity 鍵送出（ble_sm.c:2365-2426 の Identity Info/
Address＝ble_sm_tx 2回）が丸ごと `#if MYNEWT_VAL(BLE_HS_PVCY)` ガード下で，我々のビルドは
BLE_HS_PVCY=0（riscv-none-elf-gcc -E で実効値直接確認）＝コンパイルアウト**。かつ SC では
ble_sm_key_dist が ENC フラグをクリア（ble_sm.c「Encryption info … only sent in legacy
pairing」→`if(F_SC) resp_key_dist &= ~ENC`）＝ENC ブロックもスキップ。SIGN も未要求。
∴our_key_dist=ENC|ID でも «送るものが1つも残らず» sm_tx=0。だが Pairing Response で
resp_key_dist=ID を «約束» 済み→peer は我々の Identity 鍵を待つ→デッドロック→30s SM
timeout→app に ENC_CHANGE status=13(ETIMEOUT)。

**これが «共通 host config» の問題である＝C3(blob dfdadb9d)と C5(blob 015db3db)が blob
非依存で «厳密同一» に失敗した理由（両者 BLE_HS_PVCY=0 を共有）。stock IDF が同一 C3
ボードで bond 成功するのは PVCY を有効化しているから。** RX 経路比較エージェントの
「controller 側」結論は，この «config 駆動のコンパイルアウト»（esp_nimble_hci.c の RX diff
では見えない・ble_sm.c の #if で発現）を見落としていた＝相関（RXソース同一）から
因果（真因は別ファイルの #if）を早合点しない教訓の実例。

**修正の正本＝working S3**：`esp32_s3/wifi/bt/stub/include/bt_nimble_config.h:202`
`#define CONFIG_BT_NIMBLE_HS_PVCY 1`（同ヘッダのコメントが「PVCY=0 だと ble_hs_pvcy.c 全体が
#if で消え ble_hs_pvcy_our_irk 等が undefined になる」と同現象を記録）。∴C5/C3 も
...
#### C3 実機再テスト（同日・中断）：PVCY修正版を flash・広告確認（ASP3-C3-BLE <MAC-21> @ttyACM5）まで完了、connect 不可で中断
...
（BlueZ で remove <MAC-21>→power cycle→scan→connect，または nRF/スマホで試行）
...
### ★広告非到達 デバイス側判別ラウンド（2026-07-15, board B <MAC-21> @ttyACM3）：Direct Boot 健全・**adv rc=0 到達**を無条件マーカ＋sentinel で確定＝フェーズA「SYNC未到達」は board 状態依存の一過性
前ラウンド（フェーズA・scratchpad/phaseA_findings.md）は board B で新旧両ビルドとも
`0x60008050 SYNC=0`・`0x60008054 alloc=0` を観測し「ble_hs sync 未到達」と結論したが，
**「アプリがそもそも Direct Boot（download-latch 脱出）しているか」自体が未確定**だった
（0xBC=0x13121312 は ROM が usb-reset 時に上書きする stale＝esp_shim.c:1460 で確認済＝
起動証拠に使えない）。本ラウンドは無条件起動マーカで «latch/controller/sync» を判別。

#### 計装（apps/ble_host_smoke/ble_host_smoke.c・`#ifdef TOPPERS_C3_BOOT_TRACE` 既定OFF・非回帰）
`BOOT_TRACE(nn)`＝STORE2(0x60008058)へ `0xB00000nn` を無条件上書き（到達最遠点が残る）。
7点：1=main_task entry（esp_shim_initialize 前）／2=clock_init 後／3=controller_init
直前／4=init OK／5=enable OK／6=nimble_init OK／7=host task 起動（sync待ち）。既存
0x50(sync 0x5ADE51C0)・0x54(bt_shim alloc trace 0xA1020805＝init 中に発火)・0x5C(adv
0x0ADE5000)・0xC4(adv-rc 0xAD0000|rc) と併用で停止点を細分。0x58 は app が
storm_probe=0 のとき何も書かない（esp_shim.c:1443 gate）＝計装専用で安全。

#### sentinel でヌル結果を自己検証（read 経路の妥当性証明）
cbr.sh をブート前に 0x58 へ 0（クリア）ではなく **sentinel 0xCA5E0058** をプリロード
する方式へ改修。読み後の三分岐が曖昧さゼロになる：`0x58=sentinel`→main_task 未到達
（latch）かつ RTC survive/read 経路健全／`0x58=0`→測定無効（RTC 非生存 or 書込み不達
＝latch と誤読しない）／`0x58=0xB00000nn`→起動。ビルド前に objdump で
`lui a5,0xb0000`＋`# 60008058` の store が main_task に在ることを確認（マクロ伝播の
サイレント失敗＝偽 latch を排除）。

#### 実機結果（cleared-boot-read・GCC13.2・ESP32C3_QEMU=OFF）
| build | 0x58 起動 | 0x54 alloc | 0x50 SYNC | 0x5C ADV | 0xC4 adv-rc | 判定 |
|---|---|---|---|---|---|---|
| ble_host_smoke+boot-trace（2/2） | **0xB0000007** | 0xA1020805 | 0x5ADE51C0 | 0x0ADE5000 | 0xAD000000 | **adv rc=0 到達** |
| c3_conn_pvcy1（前ラウンド SYNC=0 と読んだ同一bin, 2/2） | (計装なし) | 0xA1020805 | 0x5ADE51C0 | 0x0ADE5000 | 0xAD000000 | **adv rc=0 到達** |

- verify 段で毎回 0x58=sentinel・他=0 を確認（クリーン測定）→ブート後に実値へ。
- **分類確定（デバイス側）**：①latch でない（0x58=0xB0000007＝host task 起動まで到達）・
  ②controller_init/enable 停止でない（alloc trace 発火・progression 7>5・enable 通過）・
  ③sync 停止でない（0x50=SYNC 到達）。**両ビルドとも ble_gap_adv_start rc=0（adv_active）**
  ＝広告非到達は «デバイスの boot/controller/host/adv-start» では説明されない。
- **フェーズA「SYNC=0 両ビルド」を上書き**：**同一の c3_conn_pvcy1 バイナリ**が前ラウンドは
  SYNC=0/alloc=0，本ラウンドは SYNC+adv rc=0（同手順・同ボード・2/2）。∴フェーズAの零は
  «ソース回帰» でも «恒久 controller 停止» でもなく **board B 状態依存の一過性**。
  ★因果は未確定（相関≠因果）＝2説を両論併記し断定しない：
  (a) board 状態が回復した（前ラウンドの alloc=0＝esp_intr_alloc 前の init-level 停止は
      C5/C6 の cold-boot phy-init ハング族と整合する «実在の失敗モード»。本ラウンドで
      回復）／(b) 前ラウンドの read 経路が無効だった（本ラウンドは sentinel で read 妥当性を
      «今回分» 実証したが，前ラウンドの read が無効だったことは証明しない）。sentinel は
      今回の妥当性のみ証明。

#### ユーザー保留項目（デバイス側では決着不可・hci0/スマホ/物理電源断は本エージェント範囲外）
1. **★最優先＝いま OTA 再スキャン**：board B に adv 可能ビルド（c3_conn_pvcy1）を flash 済み
   ＝いま `bluetoothctl scan le` / スマホで **ASP3-C3-BLE を再探索**。adv rc=0 が実測済＝
   «非到達» 症状は既に消えている可能性が高い（過去も OTA 検出→SYNC=0→今 rc=0 と往復）。
   再スキャンで «見える» なら症状消滅で決着。**見えない場合に限り** RF 放射層
   （adv_start は成功するが電波が届かない＝C3 AGC/RF-cal Direct Boot 族）を疑う。
2. **物理電源断→cold boot 再測定（warm 残留の判別）**：★C6 前例は naive 予測を «反転»
   させる＝MEMORY「ASP3-C6 は warm 残留に暗黙依存・cold boot で失敗」。C3 が同型なら
   物理電源断（cold）は改善ではなく «悪化» もあり得る＝この実験は «warm残留への依存 vs
   warm残留による害» の判別であって一方向の確認ではない，と解釈すること。USB 抜き差しは
   ユーザー手動。
...
**結果（board B `<MAC-21>`・regdiag build flash・cleared-boot-read）**：
| addr | 実測 | デコード |
|---|---|---|
...
board B 最終状態（更新）：regdiag build（機能同一・広告中・`0x600080C0`/`0xB8` に計装マーカー）を flash 済み。計装 `TOPPERS_C3_GATTS_REGDIAG` 既定OFF＝通常ビルド非回帰。
...
### ★申し送り（2026-07-15 セッション末・未解決）：C3+C6 が両スマホから不可視——「現在放射」は未確認のまま

**症状**：C3 D-2c/D-2d フル達成（0xABF1〜4 OTA・上記）直後、**C3(`ASP3-C3-BLE`/board B `<MAC-21>`)＋C6(`ASP3-C6-BLE`/board C `<MAC-03>`)の両方が、Android/iPhone の両方から不可視**になった。数分前は同じC3が同じスマホで見えて BT4-OK まで読めていた。

**★最重要の方法論訂正（アドバイザ確定）**：**LP_AON の adv-rc マーカ（C3 0x6000805C / C6 0x600B1008・0x600B100C）はリセットを跨いで残る＝「過去のいずれかのブートが adv_start に到達した」ことしか証明せず、「今この瞬間に放射している」証明には決してならない**。本セッションでこのマーカを「現在広告中」の根拠に使っていたのは論理ギャップ。ユーザーの「両スマホから不可視」の方が現在の放射について強い実測証拠。**マーカ再読み（usb_reset→download→LP_AON survive）で healthy 値が出ても、それは reset-survive の残存値であり現在放射の証明にならない**。

**平凡な二要因の同時発生が最有力**（単一のRF/AGC説に飛ばない）：
- **C3**：直近で board B に触れていない＝故障でなく «接続ライフサイクル» の公算。①スマホが今も接続中（connected peripheral は advertising せず scan に出ない）／②切断後に再広告していない——だが**コード確認で②は反証**（`apps/ble_host_smoke/ble_host_smoke.c:531` DISCONNECT で `start_advertising()` を呼ぶ・CONNECT でも 514 で再開）。∴①「まだ接続中」が残る最有力。
- **C6**：SM版は OTA 未確認（過去の hci0 検出は別ビルド/別セッション）。加えて本セッションで board C に usb_reset/RTS を繰り返した＝C6 warm残留/cold-boot 脆弱性（MEMORY）の交絡。

**次にやること（マスター判別＝推測でなく実測を先に）**：各ボードで **cleared-boot-read**——
1. adv マーカを 0 に write（C3 `0x6000805C`／C6 `0x600B1008`）→ read で 0 を検証（クリーン測定の担保）
2. **1回だけ** RTS/hard_reset で制御ブート → 数秒待つ
3. adv マーカを read：**`0x0ADE5000` に戻れば «今回の» ブートが adv_start 到達／0 のままなら現在ブートが adv 前でハング**（C6 の pre-adv ハングは stale マーカだと隠れる）
4. 直後（クリーン初期広告・未接続の状態）に、BT を toggle し直した片方のスマホで **その1名だけ**をスキャン
- 判定：出る→ボード健全・先の暗転はライフサイクル/キャッシュ／fresh-boot が adv 到達しても両スマホで出ない→**そこで初めて** RF放射層(C3 AGC/RF-cal Direct Boot族・§MEMORY)/スマホ側を疑う資格を得る。

**未取得の «無料» 情報（次セッションでユーザーに聞く）**：(i) 今 `ASP3-C3-BLE` はどちらかのスマホで «接続中/ペア済み» と表示されているか（BT4-OK を読んだ同じスマホか）／(ii) BT4-OK と «不可視» の間にスマホへ何をしたか（BT OFF/ON・forget・機内モード等）。

**ボード最終状態**：board B=regdiag build（`TOPPERS_C3_GATTS_REGDIAG`・機能同一）／board C=`build/c6bt_idf61_sm`（C6 D-2c/D-2d版）。両者とも «直近のブートは adv rc=0 到達» の LP_AON 値を持つが、上記のとおり現在放射は未確認。

---

### ★続報（2026-07-16）：C6（board C）の「不可視」の真因＝adv_start未到達（PHY-initハング）を実測確定。D-1〜D-2d「達成」は現在再現しない

上記「申し送り」を受け、C6 board C をこの PC で cold boot／RTS reset 計8回（真の物理電源断1回＋RTSパルスリセット7回）実測した結果、**8/8 で `esp_bt_controller_enable(BLE)` 直後の `phy_init`（`register_chipv7_phy` 内 `ram_set_chan_freq_sw_start` の RF シンセロック待ち）でハングし、adv_start に一度も到達しなかった**。つまり C6 が「不可視」だったのは接続ライフサイクルやスマホ側キャッシュの問題ではなく、**そもそも今日この PC でテストした限りではブートが advertising まで到達していない**ことが直接の原因。

詳細（v9(idf61)+SM のクリーン再ビルド・link確認、真cold/RTS reset 8/8 ハング再現、hal(v8) wifi_scan 対照実験でボード非劣化を確認、JTAGレジスタ読取りでbt_shim.cの既知3修正が全て正しく適用済みにもかかわらずRFシンセロックビット`0x600a00cc`bit8が0のまま、という診断結果）は `docs/ble-c5c6-plan.md` §16 に記録。C3側の「接続ライフサイクル」仮説はC6には無関係（C6はそもそも広告開始前でハングしているため、接続中/未接続の区別以前の段階）。

§13.5 の最も薄い D-1 専用ビルド（NimBLE/SM 無し、`build/c6bt_idf61_d1`）も本ラウンドで再ビルド・RTS reset 3回で追試したが、**3/3とも同一箇所でハング**——SM/NimBLE ビルド固有の退行という可能性も排除された。**board C 現在の flash＝`build/c6bt_idf61_d1/asp_flash.bin`**（本ラウンド最終確認ビルド）。§13.5〜§15.2 が報告した「D-1/D-2a/D-2b 達成」は、ソース差分ゼロの同一ツリー・§13.5と同一構成のビルドからも本ラウンドでは再現しなかった（詳細は `docs/ble-c5c6-plan.md` §16.7/§16.8 の解釈（a)(b) を参照）。次段は WiFi 側で90ラウンドかけて解決した RF/AGC 較正の教訓（`docs/wifi-shim-c6.md` 実施22/36-91）を BT 経路へ体系的に転写する調査が必要と見込まれる。
