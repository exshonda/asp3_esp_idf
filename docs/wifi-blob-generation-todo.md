...
| `asp3/target/esp32c5_espidf/esp_bt.cmake` L297-299 | C5 BLE(既定・唯一) | `${IDF}/components/{bt/controller/lib_esp32c5,esp_phy/lib,esp_coex/lib}/esp32c5`（`IDF=$HOME/tools/esp-idf-v6.1`） |
...
| `asp3/target/esp32c6_espidf/esp_bt_idf61.cmake` L279-281 | C6 BLE(`ESP32C6_BT_IDF61=ON`，**実機稼働中の版**) | `${IDF}/components/bt/controller/lib_esp32c6/esp32c6-bt-lib/esp32c6`＋`esp_phy,esp_coex/lib/esp32c6`（`IDF=$HOME/tools/esp-idf-v6.1`） |
...
- **BLE（トグルON＝v6.1版）は，物理的に別ファイルの`libphy.a`／
  `libble_app.a`を参照している**（`$HOME/tools/esp-idf-v6.1`配下）。
  本環境（クラウド/CIサンドボックス）にはこのパスは**存在しない**
  （`ls $HOME/tools/esp-idf-v6.1` → No such file or directory，
...
1. **C5がまさに撤去した依存の再導入**——`docs/c5-bringup.md`実施52は
   「`esp_wifi.cmake:85`の`$HOME/tools/esp-idf-v6.1`ハードコード
   依存でポータビリティ（＝統一の主目的）を損ねる」ことを理由に
   v9（IDF v6.1版WiFi）を**完全削除**した。本レビューでも
   `$HOME/tools/esp-idf-v6.1`が本環境（クラウド/CIサンドボックス）
...
- 現状C5のBLE（常時）とC6のBLE（`ESP32C6_BT_IDF61=ON`，実機で実際に
  稼働中のビルド）は`$HOME/tools/esp-idf-v6.1`というローカルパスに
...
- `$HOME/tools/esp-idf-v6.1`の非存在を`ls`で実測確認（本環境）
