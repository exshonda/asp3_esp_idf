...
**日付**：2026-07-14　**対象**：ESP32-C6 board C（`<MAC-03>`，
...
- 実機（board C，`<MAC-03>`）：2ブート独立実行で判定基準
  (a)〜(e)全て満足，出力完全一致。
...
**日付**：2026-07-14　**対象**：ESP32-C5 board C5#1（`<MAC-37>`，
...
**JTAG診断（OpenOCD，`adapter serial <MAC-37>`）**：
...
- **D-2b＝達成**：`ble_gap_adv_start`が**rc=0**で完走（STORE3
  ...
  （`<MAC-03>`，RSSI -82）を実機検出**——C6 BLE史上初の
  over-the-air adv到達。
...
`[NEW] Device <MAC-03> ASP3-C6-BLE`を確認（RSSI -82）。
同時に`ASP3-C3-BLE`（`<MAC-21>`，別セッションの既存advertise
...
**日付**：2026-07-14　**対象**：C5#1（`<MAC-37>`，UART=`b04e3bcf…`／
...
OpenOCD（`adapter serial <MAC-37>`／`board/esp32c5-builtin.cfg`／
...
**日付**：2026-07-14　**対象**：C5#1（`<MAC-37>`、UART=`b04e3bcf…`
＝ttyUSB0／JTAG=`<MAC-37>`のUSB-Serial/JTAG＝ttyACM2）　**目的**：
...
- **D-2b＝達成**：`ble_gap_adv_start`が**rc=0**（STORE2`0x600B1008`＝
  ...
  `bluetoothctl scan le`で`ASP3-C5-BLE`（`<MAC-37>`、RSSI -63〜-68）を
...
**③hci0実機検出（2ブート独立）**：ホストhci0（`<MAC-33>`）の
`bluetoothctl scan le`で`[NEW] Device <MAC-37> ASP3-C5-BLE`を
**boot1（RSSI -63〜-68）・boot2（キャッシュを`bluetoothctl remove`で消してから
再スキャン＝鮮度保証、RSSI -62〜-83）の両方で検出**。MACはC5#1のBASE MAC
（`<MAC-37>`）と一致（`own_addr_type=0`＝public address）。RSSIが
ブート間・時系列で変動することが実放射（キャッシュヒットでない）の傍証。
この`<MAC-37>`はこれまでBLEを一度も送出していない個体のため、
...
**日付**：2026-07-15　**対象**：C3（`<MAC-19>`，ttyACM3）・C5#1
（`<MAC-37>`，ttyACM1）　**出発点**：C6実施04（本ドキュメント外・
