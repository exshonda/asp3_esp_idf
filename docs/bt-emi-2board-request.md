...
**依頼元**: ESP32-S3 FMP3移植プロジェクト（`$HOME/TOPPERS/ESP32/esp32_s3`, ブランチ `feat/xtensa-esp32s3-arch`）
...
|---|---|---|---|
| 基準機A（real ESP-IDF + NimBLE, enable成功構成） | M5Stamp C3U Mate #1 | **`/dev/ttyACM3`** | ESP32-C3 rev0.3, MAC `<MAC-23>`（by-id: `usb-Espressif_USB_JTAG_serial_debug_unit_60:55:F9:57:C9:88-if00`） |
| 被験機B（FMP3 + bt_smoke, emi.c:164再現） | M5Stamp C3U Mate #2 | **`/dev/ttyACM4`** | ESP32-C3 rev0.3, MAC `<MAC-21>`（by-id: `usb-Espressif_USB_JTAG_serial_debug_unit_60:55:F9:57:C2:60-if00`） |

> ※ポート番号（ttyACM*）は接続順で変動しうる。2台とも同型のため、**MAC/by-idで個体を
> 識別**すること（A=`<MAC-23>` / B=`<MAC-21>`）。安定運用には
...
- S3側の関連知見（`$HOME/TOPPERS/ESP32/esp32_s3/wifi/debug/JTAG_DEBUG.md`）:
...
## 8. 【追記 2026-07-08】S3側が独立に enable 到達＝ブロッカー共有を確認

S3プロジェクトで、C3のBTシム資産を移植した `bt_smoke` 相当が**実機でビルド・リンク・
起動し、`esp_bt_controller_init` 成功→`esp_bt_controller_enable(BLE)` 到達**まで確認できた
（S3リポジトリ commit 40bf9ca、`wifi/debug/JTAG_DEBUG.md` 追記28）。実機ログ:
```
bt_smoke: esp_bt_controller_init
esp_shim: task 'btController' -> tskid 2   (コントローラタスク起動)
I (10) BLE_INIT: Bluetooth MAC:            (blob稼働)
bt_smoke: esp_bt_controller_enable(BLE)    (enable到達→以降停止)
```
→ **S3もC3と同じ enable 内部の地点で停止**（emi.c:164 と推定）。これにより:
- ブロッカーが **C3/S3 で共有**であることが実機で裏付けられた（同一コントローラ系列）。
- C3で真因（欠落初期化シーケンス）を特定・修正できれば、**S3にほぼそのまま移植可能**。
- S3側の arch移植（INTMTX・クロック・PHY・XIPリンク）は完了済みで、**残るは enable 内部
  の emi ブロッカーのみ**。この2台比較調査の成果を最優先で待っている状態。

S3側で流用可能を確認した知見（C3修正時の参考）:
- INTMTX ベースは C3/S3 同一（0x600C2000）。ただし S3(Xtensa) は INTENABLE を別途立てる
  必要あり（`ena_int`）。C3(RISC-V)では不要だった差分。
- controller_init 前の BBPLL/PLL クロック設定が S3 では必須（PHY較正ハング回避）。
  C3で「クロックが概ね生きていた」箇所が、S3では明示初期化を要した。emi の真因が
  「enable前後の初期化シーケンス欠落」なら、この種のクロック/較正初期化が候補になりうる。
