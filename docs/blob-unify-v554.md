...
set(IDF_V554 $HOME/tools/esp-idf)   # 未定義なら既定値
...
各chipとも`build.ninja`の`-L`実体で`$HOME/tools/esp-idf/...`が
実際にリンクされていることを機械確認済み（パス推測ではない）。

### C3（`<MAC-19>`，usbjtag console）
...
### C5（`<MAC-39>`，UART0 console＝`/dev/ttyUSB4`）
...
if(ASP3_BT_IDF_V554)
    set(IDF $HOME/tools/esp-idf)
else()
    set(IDF $HOME/tools/esp-idf-v6.1)
endif()
```
...
**C5（`bt_smoke_c5`，D-1コントローラのみ．board C5#2 `<MAC-39>`）**：
...
`<MAC-03>`）**：build成功（RAM 65.86%）。RTSリセットによる
...
flash→真の電源断（off 10s→on）→BlueZ scan で **`ASP3-C5-BLE`（<MAC-39>）が cold
...
### §12.3 実機 A/B（board `<MAC-19>`，同一 host/session，back-to-back）
...
**`ASP3-C5-BLE`（<MAC-39>）が cold から adv 放射**。
...
- **現在のコードとの関係**：`asp3/target/esp32c3_espidf/target.cmake` は既に
  **`IDF_V554` の既定を submodule `esp-idf/`（真のv5.5.4）へ移してあり**，
  外部絶対パス `$HOME/tools/esp-idf` への依存は撤去済み
