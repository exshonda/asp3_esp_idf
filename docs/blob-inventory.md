...
| 供給元 | 実体 | pin/版 |
|---|---|---|
| hal submodule | `$HOME/TOPPERS/asp3_esp_idf/hal`（`.gitmodules`の`hal`） | commit `b90b1837cb5ad24747deb4c895246037cc206ce5`（`sync/master.c-nuttx-20260428-24-gb90b1837cb5`，`git submodule status`実測） |
| ESP-IDF v6.1 | `$HOME/tools/esp-idf-v6.1`（ローカルパス直参照，submodule化されていない） | `esp_idf_version.h`文字列 = 6.1.0（下記） |

両者とも `esp_idf_version.h` の文字列は **同一**（`ESP_IDF_VERSION_MAJOR/MINOR/PATCH`
= `6`/`1`/`0`）——実測：

```
$ grep -n VERSION_M $HOME/TOPPERS/asp3_esp_idf/hal/components/esp_common/include/esp_idf_version.h
