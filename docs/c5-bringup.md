...
1. `phy_iq_est_enable_new`が起動ビットを立てた後，**BBがIQ推定を完了できない理由**を追う。
   FE 20M/40Mは棄却済み。次候補：(a) BBがクロック供給を受けた**タイミング**が遅すぎる
   （blobは`wifi_clock_enable`より前にBBを触っている可能性＝ICG初期化をさらに早い時点，
   例えばWi-Fi初期化の最初やDirect Boot直後に移す），(b) RF/PLL（regi2c経由）の較正が
   未完了でBBがサンプルを得られない，(c) `MODEM_SYSCON_WIFI_BB_CFG`(`0x600a9c18`
   ＝`0x10003802`)や`FE_CFG`(`0x600a9c1c`＝`0x00000000`)の設定欠落。
   **特に(a)は安価に試せる**（`esp_shim_modem_icg_init()`の呼出し位置を前倒しするだけ）。
2. `esp_shim_modem_icg_init()`内の`modem_syscon_ll_set_modem_apb_icg_bitmap(syscon, BIT(2))`は，
   実測で`CLK_CONF_POWER_ST`が`0x64646400`のまま変化していない（APBニブルが6→4にならない）。
   構造体オフセットかビットフィールド定義の齟齬の可能性があり，**要確認**
   （本ラウンドのA/B実験ではこのマップ書換えは不要と判明しているため実害は無いが，
   意図した動作をしていない疑いがある）。
