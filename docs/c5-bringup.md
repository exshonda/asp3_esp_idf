...
- **DUT＝ESP32-C5 #1**（主機）。chip **ESP32-C5 rev v1.0**（esptool実読）。
  - JTAG/usbjtagコンソール＝native USB-Serial-JTAG：**MAC `<MAC-37>`**（VID:PID 303a:1001，通常 `/dev/ttyACM2`・変動注意）。
  - UART（CP2102Nブリッジ）：**serial `b04e3bcfa270f0118f4894301045c30f`**（通常 `/dev/ttyUSB1`・変動注意）。
  - **番号は挿抜で変動。必ずMAC/シリアルで照合してから触る**（`~/usb_status.md`）。
  - **触れない同居ボード**：S3-A（JTAG MAC `<MAC-40>`）・P4-A（JTAG MAC `<MAC-08>`）・その他。
...
- OpenOCD：`~/tools/espressif/tools/openocd-esp32/v0.12.0-esp32-20260424/openocd-esp32/`（binと同梱scripts）。
  **必ずピン留め**：`-c "adapter serial <MAC-37>" -f board/esp32c5-builtin.cfg`。
...
（JTAG MAC `<MAC-37>`／ttyACM2）へのOpenOCD JTAG（`adapter serial`ピン留め）と，
...
- `build/c5_wifi_scan_usbjtag/asp_flash.bin` を esptool v5.3.1（`--chip esp32c5`）で
  `/dev/ttyACM2`（MAC照合済み `<MAC-37>`）へ 0x0 書込み・`Hash of data verified`。
...
OpenOCD（`adapter serial <MAC-37>`ピン留め）で10回連続halt＝**PC=`0x42020C0C`
...
- `build/c5_wifi_scan_usbjtag/asp_flash.bin`を esptool v5.3.1（`--chip esp32c5`）で `/dev/ttyACM2`
  （MAC照合 `<MAC-37>`）へ 0x0書込み・`Hash of data verified`。
...
```
JTAG（`adapter serial <MAC-37>`）でhalt：**PC=`0x42020C0C`（dispatcher_1アイドル）・
...
- 実機は2台：**C5 #1＝ASP3 DUT**（JTAG MAC `<MAC-37>`／今回はttyACM4，UART bridge=ttyUSB2）と
  **C5 #2＝stock参照**（JTAG MAC `<MAC-39>`／ttyACM2，UART bridge=ttyUSB1）。毎操作前に
...
- **実機（C5 #1 = <MAC-37>，クリーン再書込み11/11再現）**：
...
**ESP-IDF v6.1（v9・os_adapter 0x09．`$HOME/tools/esp-idf-v6.1`）へ移行**．
...
- **ESP-IDF v6.1 を `~/tools/esp-idf-v6.1` に配置**（移行必須）。`esp_wifi.cmake:85` が
  `set(IDF $HOME/tools/esp-idf-v6.1)` と**ローカルパス直書き**．別パスなら同行を書換えるか
...
- 実機：C5 #1（DUT）=JTAG MAC `<MAC-37>`・UART `b04e3bcf…`／C5 #2（stock v9参照）=
  `<MAC-39>`・`3e7bd19f…`．**番号でなくMACで照合**（`~/usb_status.md`）．触れない機＝
  S3-A `<MAC-40>`／P4-A `<MAC-08>`．esptool＝idf6.1 venv v5.3.1，OpenOCDは
...
- **esptool**：`idf6.1 venv v5.3.1`は本マシンに無かったため，既存のIDF v5.5用venv
  （`$HOME/tools/python_env/idf5.5_py3.12_env`）のesptool（v4.12.dev3）で代替。
  引数形式がハイフンでなくアンダースコア区切り（`--before usb_reset`・`write_flash`）である点に
  注意（バージョン差）。
- **実機**：接続時に**C5#1（DUT）ではなくC5#2（stock参照）が挿さっていた**ことを検出
  （MACアドレス`<MAC-39>`で判別）。C5#1（`<MAC-37>`）が追加接続されるまで
  DUTへの操作は待機した。
...
- 実機（C5#1，`<MAC-37>`）：書込み・起動・JTAG確認。0x3001即時失敗は再現せず。
...
4. C5#2（stock参照，`<MAC-39>`）は本ラウンドも一切接続・操作せず（未接触）。
...
- C5#2（stock参照，`<MAC-39>`）：本ラウンドは一切接続・操作せず（未接触を確認）。
...
- C5#2（stock参照，`<MAC-39>`）：本ラウンドも一切接続・操作せず（未接触）。
...
- 実機（C5#1，`<MAC-37>`）：UARTブリッジ（ttyUSB1）RTSリセットによる
...
- **Gate 1（blob同一性）**：ASP3の`esp_wifi.cmake:85`は
  `set(IDF $HOME/tools/esp-idf-v6.1)`で**同一ツリーを直接参照**
...
[  1.151] I (1112) wifi:mode : sta (<MAC-37>)
...
- 実機（C5#1，`<MAC-37>`）：
  ...
  - 最終復帰：ASP3イメージのまま（実施13から無変更），UARTブリッジRTS
    ...
    確認——復帰完了。
- C5#2（`<MAC-39>`）：本ラウンドも一切接続・操作せず（未接触，
  MACで都度照合）。
...
- 実機（C5#1，`<MAC-37>`）：
  ...
  - 最終復帰：ASP3計装ビルドのまま，UARTブリッジRTSリセットで約3.5秒周期の
    ...
    復帰完了。
- C5#2（`<MAC-39>`）：本ラウンドも一切接続・操作せず（未接触，MACで
  都度照合）。
...
- 実機（C5#1，`<MAC-37>`）：
  ...
  - 最終復帰：ASP3計装ビルドのまま，UARTブリッジRTSリセットで約3.5秒周期の
    ...
    完全一致，復帰完了。
- C5#2（`<MAC-39>`）：本ラウンドも一切接続・操作せず（未接触，udevadm
...
**本ラウンドもC5#1（`<MAC-37>`）のみを使用，C5#2には一切接続・操作していない**（udevadm
...
- **C5#1（`<MAC-37>`）のflash内容は不確定**（実施21エージェントを
...
- C5#2（`<MAC-39>`，stock v9参照機）は実施14〜21を通じて一切未接触。
...
| JTAG用USB-JTAG-serialのttyACM番号（`/dev/ttyACM1`/`ACM2`等）はUSB再列挙順に依存し，docs記載の固定値と実際の対応が入れ替わることがある（実施41で`<MAC-37>`がACM1ではなくACM2で列挙された実例を確認） | ttyACM/ttyUSB番号を信用せず，`/dev/serial/by-id/`のMACアドレス付きパスのみで照合してから操作する（UARTブリッジのby-idパスは本セッション内では安定していた） | 実施41 |
...
**本ラウンドもC5#1（`<MAC-37>`）のみ使用。C5#2（`<MAC-39>`）は物理切断済みで
...
本ラウンドもC5#1（`<MAC-37>`，ttyACM0／UARTブリッジttyUSB0）のみ使用。
...
- C5#2（`<MAC-39>`）：本ラウンドも物理切断済み・一切未接触。
...
（`<MAC-37>`，ttyACM0／UARTブリッジttyUSB0）のみ使用。C5#2は物理切断済みで
一切未接触。
...
- `ttyUSB0`：C5#1のUARTブリッジ（`esptool chip-id`で`<MAC-37>`と実機確認——
  docs記載のDUTと一致）。
- `ttyACM1`/`ttyUSB1`：**C6 AGC調査（`memory/project_c6_agc_investigation.md`，
  ★FROZEN at 実施85）の「board C」（JTAG MAC`<MAC-03>`，`esptool chip-id`で
...
- C5#2（`<MAC-39>`）：本ラウンドも物理切断済み・一切未接触。
- C6 board C（`<MAC-03>`，別調査★FROZEN）：本ラウンドは`esptool chip-id`
...
トレース」の実施回。本ラウンドはC5#1のnative USB-JTAG（`<MAC-37>`）が
...
- 本ラウンドのリポジトリ差分はarch 3ファイルの**コメント＋レジスタ定義
  2行のみ**（機能変更なし。ビルド確認済み）。
- C5#2：物理切断のまま未接触。C6 board C（`<MAC-03>`）・その
...
- C5#2：物理切断のまま未接触。C6 board C（`<MAC-03>`＝ttyACM0）
...
- 本ラウンド終了処理として同じ`build/c5_idf61_uart/asp_flash.bin`を
  ...
  較正ハングの標準症状を再確認。
- C5#2：物理切断のまま未接触。C6 board C（`<MAC-03>`）・
  UARTブリッジ`125a266b...`：`/dev/serial/by-id`一覧での識別以外
...
`<MAC-37>`（本セッションでは`/dev/ttyACM1`）を使用。
...
（本セッションでは`/dev/ttyUSB0`）／JTAGは`<MAC-37>`（本
...
- C5#2・C6 board C（`<MAC-03>`）・UARTブリッジ`125a266b...`：
  完全未接触。DUTは常にUARTブリッジ`b04e3bcf...`（本セッション
  /dev/ttyUSB0）／JTAG `<MAC-37>`（/dev/ttyACM1）をMAC/
  シリアル照合の上使用。
...
を優先順位どおり実行。C5#1（`<MAC-37>`，UARTブリッジ`b04e3bcf…`）
のみ使用，禁止個体には未接触。
...
C5#2（`<MAC-39>`）を**ユーザーが2026-07-13に直接承認**した役割変更で
スニファへ転用し，C5#1（`<MAC-37>`）のscan中probe requestが空中に
出ているか（TX）・promiscuousコールバックが発火するか（RX）を初めて直接測る。

### 0. 環境・機材

- C5#1（DUT）＝`<MAC-37>`／ttyACM1／UARTブリッジ`b04e3bcf…`＝ttyUSB0。
  着手時flash＝`build/c5_r34_wifi80`（240MHz出荷構成，実施34以降の標準）。
- C5#2（スニファ，**役割変更をユーザー承認済み・2026-07-13**）＝
  `<MAC-39>`／ttyACM2／UARTブリッジ`3e7bd19f…`＝ttyUSB2。
  着手時flash＝旧stock scanサンプル（上書き）。
- 接触禁止（C6 board C `<MAC-03>`／ttyACM0，UARTブリッジ`125a266b…`／
...
送信元MAC＝DUT（`<MAC-37>`）のprobe request数・全フレーム数を
...
- C5#1（DUT）＝`<MAC-37>`／UARTブリッジ`b04e3bcf…`＝ttyUSB0。
  着手時flash＝`build/c5_r34_wifi80`（240MHz出荷構成）@0x0，0x200000消去済み
  （実施38終了状態のまま）。
- C5#2（スニファ）＝`<MAC-39>`／UARTブリッジ`3e7bd19f…`＝ttyUSB2。
  実施38の2.4GHz ch6スニファfirmwareのまま流用（本ラウンドは書換え一切なし，
  RTSリセットのみ）。
- 接触禁止2台（C6 board C `<MAC-03>`／ttyACM0，UARTブリッジ
...
- 終了処理：C5#1を`build/c5_r34_wifi80`（240MHz出荷構成，実験パス無し）
  ...
  のまま未書換え・維持。接触禁止2台（C6 board C `<MAC-03>`／
...
`ACM2`番号がdocs記載値と入れ替わって列挙されていた（`<MAC-37>`
...
接触禁止2台（`<MAC-03>`・UARTブリッジ`125a266b…`）は全工程で
...
- 接触禁止2台（C6 board C `<MAC-03>`／UARTブリッジ
...
C5#1（`<MAC-37>`，JTAG=ttyACM2・UARTブリッジ`b04e3bcf…`=
ttyUSB0）・C5#2（`<MAC-39>`，JTAG=ttyACM1・UARTブリッジ
`3e7bd19f…`=ttyUSB2，実施38以来2.4GHz ch6スニファのまま）を`udevadm`/
`esptool chip-id`のMAC照合で確認。接触禁止2台（JTAG `<MAC-03>`
...
- **C5#1**：`build/c5_r43_shipping/asp_flash.bin`
  ...
  ハンドオフ検証時は再書込みが必要）。
- **C5#2**：スニファ（2.4GHz ch6版，実施38以来）のまま未書換え・維持。
- 接触禁止2台（JTAG `<MAC-03>`／UARTブリッジ`125a266b…`）は
...
OpenOCD（`adapter serial <MAC-37>`）でhaltすると，OpenOCD
...
DUT＝C5#1（`<MAC-37>`，UARTブリッジ`b04e3bcf…`＝by-id照合），
...
**環境**：DUT＝C5#1（`<MAC-37>`，UARTブリッジ`b04e3bcf…`，
...
- DUT＝ESP32-C5 #1 `<MAC-37>`（native USB=ttyACM4／UART
...
1. **v8で判定点(c)**：connect→DHCP→ping→TCP/UDP＋5GHz（ch48等）を
   ...
   ローカルパス直書き（`$HOME/tools/esp-idf-v6.1`）を撤去して
...
## 実施49：★v8/hal を C5 Wi-Fi の «既定» へ昇格＋IDF v6.1 ローカルパス依存の隔離（ユーザー決定＝v6.1がbetaのためIDF非依存のv8/halへ舵）——`ESP32C5_WIFI=ON` の既定が v8（hal submodule blob・IDFパス不要）を選ぶよう反転。v9 は `-DESP32C5_WIFI_V9=ON` の opt-in fallback として温存。既定v8ビルドが `$HOME/tools/esp-idf-v6.1` を一切参照しないことを実証
...
`set(IDF $HOME/tools/esp-idf-v6.1)` ハードコードに依存する＝
...
- 生成物（`build/c5_v8_default/`）を
  `grep -rl "esp-idf-v6.1\|$HOME/tools"` → **ヒット0**＝
  既定v8ビルドは `$HOME/tools/esp-idf-v6.1` を**一切参照しない**。
...
DUT＝ESP32-C5 #1 `<MAC-37>`（MAC照合：native USB by-id
`…<MAC-37>…` = ttyACM4／UART CP2102N `b04e3b…` = ttyUSB3）。
...
**実機結果**（DUT＝C5#1 `<MAC-37>`＝MAC照合，UART `b04e3b…`／
...
### 実機結果（DUT＝C5#1 `<MAC-37>`＝MAC照合，UART `b04e3b…`／書込 native USB ttyACM4，認証情報はビルド注入・非記載）
...
- **FORCE5G RTC マーカ**（`esptool read-mem`，DUT を download mode で回収）：
  ...
  | `0x500000D4/D8` fg5/6 | `0xffc93608`/`0x630e` | 選択 BSSID＝`<MAC-01>` |
  ...
  選択 BSSID `<MAC-01>` は接続後の `<ba-add> TALO:0xffc93608
...
## 実施52：v9（IDF v6.1 blob 変種）を完全削除——hal(v8) 単一実装へ統一（ユーザー決定）。実施48(scan)・50(2.4GHz connect+DHCP)・51(5GHz connect+DHCP) で v8 の主要経路を実機実証し，gate を満たしたため

### 背景・決定

実施49 で v8/hal を既定へ昇格し，v9（IDF v6.1 blob ＝ `esp_wifi.cmake` ＋
`wifi/`）は `-DESP32C5_WIFI_V9=ON` の opt-in fallback として温存していた。
v9 は `esp_wifi.cmake:85` の `$HOME/tools/esp-idf-v6.1` ハードコード
依存でポータビリティ（＝統一の主目的）を損ねる。実施48〜51 で v8 の
scan／2.4GHz connect+DHCP／5GHz connect+DHCP を実機実証し「v9 削除前 gate」
（実施49 で設定）を満たしたため，**ユーザー決定で v9 を削除**する。

### 削除内容

- `asp3/target/esp32c5_espidf/esp_wifi.cmake`（808行，v9 の IDF依存ビルド構成）を削除。
- `asp3/target/esp32c5_espidf/wifi/`（9ファイル：`esp_shim.c`／
  `esp_shim_blobglue.c`／`esp_wifi_adapter.c`／`wifi_trace.[ch]`／
  `freertos_stub/`）を削除。使用箇所は `ESP32C5_WIFI AND ESP32C5_WIFI_V9`
  のみ（BT単体ビルドは実際には else 分岐で `wifi_v8/` を使っており，199/245行
  の「BT は wifi/ を使う」コメントは v8 既定化前の古い記述＝コード実挙動と
  食い違っていた。削除は v8/BT ビルドに無影響）。
- `target.cmake`：`ESP32C5_WIFI_V9`／`ESP32C5_WIFI_HAL_V8` オプション，
  v8/v9 変種選択の if/else 分岐（`ESP32C5_WIFI_SRCDIR`／`ESP32C5_WIFI_CMAKE_FILE`
  を常に `wifi_v8`／`esp_wifi_v8.cmake` に固定），v9専用の
  `ESP32C5_WIFI_REGI2C_TRACE` 計装（`wifi/wifi_trace.c` 依存）を削除。
  ヘッダ・BT共有コメントの stale 参照（`esp_wifi.cmake`→`esp_wifi_v8.cmake`，
  BT srcdir）も更新。

### 検証（v9削除後の既定v8ビルド）

`-DESP32C5_WIFI=ON -DESP32C5_LWIP=ON`（app=wifi_dhcp，gcc 15.2.0）で
configure/build：

- **configure 成功**（v8固定ロジックで cmake エラーなし）。
- **link 0 エラー・RAM 81.05%・FLASH 12.91%**＝実施51 の v8 ビルドと完全一致
  （±0＝非回帰）。
- 生成物を `grep -rl "esp-idf-v6.1\|esp32c5_espidf/wifi/"` → **ヒット0**＝
  IDFローカルパス・削除した v9 dir への参照ゼロ。**IDF非依存を再実証**。

### 変更ファイル（実施52）

- 削除：`esp_wifi.cmake`，`wifi/`（9ファイル）。
...
board=C5#2（`<MAC-39>`／`/dev/ttyACM4`）。**C5#1
（`<MAC-37>`／`/dev/ttyACM3`）は本ラウンド開始時点で既に
