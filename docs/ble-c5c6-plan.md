...
esp_bt.cmake`が要求するESP-IDF v6.1，`$HOME/tools/esp-idf-
...
- repo `$HOME/TOPPERS/ASP3CORE/asp3_esp_idf`．GCC=`riscv32-esp-elf
...
- DUT=C6 board C：JTAG/USB-Serial-JTAG=`<MAC-03>`（by-id，現ttyACM0），
...
- OpenOCD=`tools/espressif/tools/openocd-esp32/v0.12.0-esp32-20260703`，
  C6は`board/esp32c6-builtin.cfg`＋`adapter serial <MAC-03>`で
...
§10のPHY-initハングを実機board C（`<MAC-03>`，全ブートRTS
...
OpenOCD＝`board/esp32c6-builtin.cfg`＋`adapter serial <MAC-03>`，
...
board C（`<MAC-03>`，全ブートRTSリセット）に `build/c6bt_idf61/asp_flash.bin`
...
board C（`<MAC-03>`，全ブートRTSリセット）に `build/c6bt_idf61_nimble/
...
### 15.2 device-side 実機判定（board C `<MAC-03>`・RTS リセット・LP_AON STORE 直読み）
...
`-L$HOME/tools/esp-idf-v6.1/components/esp_phy/lib/esp32c6` の
...
`python3 $HOME/bin/usbhub3c_ctl.py off 1`→3秒→`on 1`→USB-JTAG
...
`board/esp32c6-builtin.cfg`，`adapter serial <MAC-03>`．
...
bluetoothctl scan で **`[NEW] Device <MAC-03> ASP3-C6-BLE`
...
- **C5 #2（`<MAC-39>`・port3・by-id …C8:94・ttyACM4）** を親が
...
`tmp/c6bt_stock`（ジャンプ無し）を board C（port1，<MAC-03>，
...
I (316) BLE_INIT: Bluetooth MAC: <MAC-05>
...
BlueZ で確認：`bluetoothctl scan on` で `STOCK-C6-BT`（<MAC-05>）
...
BlueZ で確認：`bluetoothctl scan on` で **`ASP3-C6-BLE`（<MAC-03>）
...
## §20.5 ★親の cold 実測（pmu_init は «一部 landed だが不十分»）＋機序の深掘り（MODEM/BBPLL アナログ給電の «即時適用» 欠落が第一候補）

親が port1 を真電源断（off 10s→on，reflash せず＝§20 の fix build cold 一発目）→
`rts_boot_capture` でクリーン採取：

- **`HANDOFF entry synth(0x600a00cc)=0x25824e50`（bit8=0）＝真に cold・test 妥当**．
- **`PMU[0x600b000c]=0x80000000`（POR 0x0 から «変化»）＝pmu_init が landed**
  （`0x600b000c`＝`PMU_HP_ACTIVE_ICG_MODEM_REG`＝ICG_MODEM code=2．pmu_init 内
  `pmu_hp_system_init`→`pmu_ll_imm_update_dig_icg_modem_code` の «即時適用» が効いた）．
- **`PMU[0x600b0010]=0`・`PMU[0x600b0100]=0`（POR のまま）**．
- → `esp_bt_controller_enable(BLE)`→`phy_init` で «ハング»．BlueZ で
  `ASP3-C6-BLE` «不在»．

**★診断アドレスの訂正（重要）**：`0x600b0010`＝`PMU_HP_ACTIVE_HP_SYS_CNTL_REG`
（uart_wakeup/pad_hold/pause_wdt/cpu_stall），`0x600b0100`＝
`PMU_POWER_PD_HPPERI_RESERVE_REG`．**いずれも «RF-アナログ電源記述子» ではなく，
HP_ACTIVE で 0 が «正しい既定値» でありうる**（pmu_init が書いても 0）．∴
「0x600b0010/0100 が POR＝アナログ未設定」という当初の読みは «不確か»．正しい
RF/MODEM 電源関連レジスタは `0x600b0000`（`DIG_POWER`＝wifi_pd_en）・
`0x600b0014`（`HP_CK_POWER`＝xpd_bb_i2c/xpd_bbpll_i2c/xpd_bbpll）．次回 cold で
これらを読む．

**★機序の深掘り（静的解析）**：
1. **C6 も `SOC_PM_MODEM_PD_BY_SW` 未定義**（C5 と同一．`SOC_PM_SUPPORT_MODEM_PD=1`
   だが `_BY_SW` 未定義）．∴`esp_phy_enable`→`esp_wifi_bt_power_domain_on()`
   （phy_init.c:453）は **C6 で本体が丸ごとコンパイルアウト＝空関数**．RTC_CNTL_
   WIFI_FORCE_PD/ISO をクリアする «SW power-domain-on» が C6 では走らない．
   ＝MODEM/RF アナログドメインの給電は «pmu_init の HP_ACTIVE 設定頼り»．
2. **pmu_init の HP_ACTIVE 設定は «正しく» RF を給電する記述子を持つ**：
   `PMU_HP_ACTIVE_POWER_CONFIG_DEFAULT`＝`dig_power.wifi_pd_en=0`（active で
   WiFi/MODEM ドメイン非パワーダウン）・`clk_power.xpd_bb_i2c=1/xpd_bbpll_i2c=1/
   xpd_bbpll=1`（**BBPLL・BB-i2c アナログ給電**）．∴設計上は pmu_init で RF が
   給電されるはず．
3. **第一候補＝«即時適用» の欠落**：`pmu_hp_system_init` は各モードの «設定バンク»
   を書くが，即時反映するのは ICG 系のみ（`pmu_ll_imm_update_dig_icg_*`）．
   `clk_power`（xpd_bbpll 等）には別途 `pmu_ll_imm_set_clk_power()` という «即時»
   経路があるが，**pmu_init はこれを呼ばない**（設定バンクへ書くのみ）．PMU は
   «HP_ACTIVE モードへ遷移する瞬間» に設定バンクを適用するが，ASP3 は既に
   HP_ACTIVE で走行中に pmu_init を呼ぶため，`clk_power` の xpd_bbpll が «即時»
   反映されず BBPLL アナログが給電されないまま＝register_chipv7_phy の regi2c
   PLL 設定が着弾しても（§18）PLL がロックしない，という筋．ICG_MODEM が
   `0x600b000c` に見えたのは «即時適用がある» から，power/clk_power が見えない/
   効かないのは «即時適用を呼んでいない» から，で整合する．
   （注：`pmu_ll_imm_set_clk_power` の stock 呼出し元はリンク対象ソースに無く
   ＝blob/ROM 側 or modem_clock 経路の可能性．未確定＝要実機確認．）

**次段（次回 cold で判定）**：
- 診断強化：entry で `0x600b0000`（DIG_POWER）・`0x600b0014`（HP_CK_POWER）を
  読み，pmu_init が xpd_bbpll/wifi_pd_en を設定バンクに書けているか確認．
- 候補 fix：pmu_init 後に `clk_power` の xpd_bbpll/xpd_bb_i2c を «即時適用»
  （`pmu_ll_imm_set_clk_power` 相当）＋C5 で使った ICG bitmap 拡張（fe/wifi/
  modem_periph/coex/wifipwr）を C6 BT へ．cold で synth bit8=1→adv→BlueZ 放射
  を目標．反証＝各試行の直前に親の真電源断．

## §20.6 ★機序の再整理（WiFi cold-hang 先例で pmu_init 方向は «立つ» が壁は BT 固有の bit8 アナログ PLL ロック）＋次の decisive＝cold Arm B ハンドオフ

親の cold 結果（§20.5）を受け，«なぜ pmu_init で不十分か» を先例で詰めた．

**（A）free チェックで判った先例（docs/wifi-shim-c6.md 実施90/91）**：
- **C6 WiFi «も» cold で phy-init ハングする**（`register_chipv7_phy`→`rf_init`→
  `wait_i2c_sdm_stable`，`0x600ad000`(MODEM_PWR0)フリーランカウンタが
  cold で 0x0 凍結）．実施90 の «scan 完走» 系は **NuttX 直後の warm handoff
  依存**だった（＝BT の §19 warm-residual と同型・同根）．∴「WiFi は cold で
  収束するから壁は BT 固有経路」という一時仮説は **先例で反証**＝«cold の
  MODEM/アナログ初期化欠落» は WiFi/BT 共通根で，pmu_init（stock cold init 移植）
  方向は «立つ»．
- **WiFi の cold fix（実施91）＝ICG code=0→2**（`esp_shim_modem_icg_init`．
  JTAG 直接注入で `0x600ad000` が 0→フリーラン化しハング脱出を一次実証．
  ただし «ソースツリー経由での cold 完走» は当時 warm 汚染下で未確定）．

**（B）ただし WiFi と BT は «ハング点が違う»（advisor 指摘・実測で確認）**：
- WiFi＝`wait_i2c_sdm_stable`（`0x600ad000` 凍結）＝ICG ゲート（code=0）が壁．
- BT＝`ram_set_chan_freq_sw_start`（`0x600a00cc` bit8）＝**アナログ PLL ロック**
  ＝SDM/regi2c より «下流»．
- **BT は既に ICG ゲートを越えている**：親 cold で `0x600b000c`=code=2 設定済み，
  §18 で regi2c 212 write 完走＝bit8 poll へ到達．本ラウンドの warm 実測でも
  **`MODEM_PWR0(0x600ad000)=0x0 «のまま» BT は enable OK まで到達**＝BT の壁は
  «WiFi の 0x600ad000 ゲートではない»．∴BT 側で 0x600ad000/ICG を «さらに»
  追う筋は WiFi の壁を撃つ空振り．BT の壁は §18 が «デジタル不可視のアナログ
  PLL ロック» と三重に裏付けた bit8＝静的レジスタ移植の伸びしろは小さい．

**（C）∴次の decisive＝cold Arm B ハンドオフ（親が §19.8 で «次の decisive» と
明示済み・本タスクで cold 未実施）**：
- 狙い：«stock の完全 cold 起動（pmu_init を超える esp_clk_init/esp_perip_clk_init
  等を含む）を継承したら ASP3 BT が cold から bit8 ロックできるか» を «推測なし» で
  一撃判定．同一 guest（pmu_init＋full PMU dump 込み）を使い，唯一の変数を
  «stock の cold 起動 prefix» に絞る．
- **board C（port1）は現在 cold Arm B を staged**：stock armB（`tmp/c6bt_armB`＝
  BT 一切未初期化・起動2秒後ジャンプ）を 0x0/0x8000/0x10000，guest
  （`build/c6ble`＝pmu_init＋entry full PMU dump）を 0x200000 へ，`--after no-reset`
  で «未起動»．親の電源投入一発目が clean cold Arm B．
- 判定（BlueZ 実放射で・マーカ単独でない）：
  - **Arm B が cold で lock→adv→BlueZ `ASP3-C6-BLE`**：欠落材料は stock cold
    起動の «pmu_init を超える» 部分にあり，かつ soft-reachable．次段＝stock
    起動の bisect（esp_clk_init/esp_perip_clk_init 等）＋entry PMU dump の
    Arm-B vs standalone 差分でレジスタを名指し．
  - **Arm B «も» cold でハング**：stock の完全 cold 起動を継承しても ASP3 の
    BT re-init で lock しない＝**standalone 複製は行き止まり，ハンドオフ自体が
    deliverable**（stock は cold で lock＝§20.5 STOCK-C6-BT．ハンドオフ経由 BT が
    出荷可能解）．
- 反証：直前に必ず親の真電源断で cold 保証（§19 教訓）．

**★親への依頼（1 電源サイクル）**：port1 を真電源断（off→数秒→on，reflash
しない）→一発目の console（entry synth＝cold なら `...e50`・full PMU dump・
`esp_bt_controller_enable` 通過可否）＋BlueZ（`ASP3-C6-BLE` 有無）を採取．
（任意 follow-up：Arm B が lock したら，同一 guest を 0x0 単独へ焼き直し cold
standalone の PMU dump を採り，Arm-B と diff＝欠落レジスタの名指しに使う．
flash 例＝`esptool ... write-flash 0x0 build/c6ble/asp_flash_trunc1M.bin`）．

**proportionality（advisor＋兄弟先例）**：C5・C6-WiFi とも同じ «cold アナログ
PLL/RF» 壁で unsolved のまま deferred．cold で stock は lock する（soft-reachable
は確定）．Arm B が «standalone 不可» を示したら，ハンドオフ BT が正当な出荷解＝
standalone レジスタ bisect に round-trip を無限投下しない．

## §20.7 ★cold Arm B が «USB-JTAG flap で inconclusive» → «reboot ループに強い LP_AON マーカ» 計装で読める化（親は esptool read-mem で読む）

親の cold Arm B（真電源断→投入，reflash せず）は **BlueZ で `ASP3-C6-BLE`/
`STOCK-C6` とも不在**，かつ **stock IDF ビルドの reset ループで USB-JTAG(ttyACM)
が flapping**＝`rts_boot_capture.py`/`cat` とも `SerialException`/hang で
**console が全く読めず inconclusive**（guest BT の hang か，handoff/stock 起動の
失敗かを区別できず）．

**対策＝console print でなく LP_AON STORE マーカ**（usb-reset/flap を跨いで
保持．親が `esptool read-mem` で読める）に到達段階＋cold synth＋真の RF電源を
記録する計装を追加した（コミット済）：

- **guest（`build/c6ble`＝`ble_host_smoke_c6`）**：
  - `LP_AON_STORE1`（`0x600B1004`，正規未使用）＝到達 stage（`0x5A6E00NN`）：
    `01`=guest main_task 到達（jump 成功）／`02`=controller_init 直前／
    `03`=controller_enable 直前（**ここで止まれば phy_init hang**）／
    `04`=controller_enable OK（**★phy_init 突破＝cold ロック成功の一次証拠**）／
    `05`=ble_hs SYNC．
  - `LP_AON_STORE0`（`0x600B1000`）＝entry synth `0x600a00cc`（cold＝
    `0x25824e50`）．※stage4 通過後は «通過後 synth» で上書き（bit8 立ったか）．
    ※stage5(SYNC) 到達時は sync マーカ `0x5ADE51C0` で上書き（＝成功系）．
  - `LP_AON_STORE2`（`0x600B1008`）＝PMU `0x600b0000`（DIG_POWER）．
    ※stage adv 到達時は adv マーカ `0x0ADE5000` で上書き（＝成功系）．
  - `LP_AON_STORE3`（`0x600B100C`）＝PMU `0x600b0014`（HP_CK_POWER＝
    xpd_bbpll 等 «真の RF電源»．pmu_init が cold で立てられているかの直接確認）．
...
**warm 自己テストで計装の着弾を機械確認**（§3.5 の «読み戻しで着弾確認»）：
guest を warm 起動（成功系）→`esptool read-mem` で `STORE1=0x5a6e0005`（stage5=
SYNC 到達）・`STORE0=0x5ade51c0`（sync マーカ）・`STORE2=0x0ade5000`（adv
マーカ）を確認＝**マーカ機構＋read-mem 読出しが機能**．cold hang（stage3）では
STORE0/2/3 の正規上書き（sync/adv）が走らないため entry 診断値がそのまま残る．

**board C（port1）は cold Arm B を staged**（marker 計装版 stock armB @0x0＋
guest @0x200000，`--after no-reset` で未起動）．

**★親への依頼（1 電源サイクル＋read-mem．USB console 不要）**：
1. port1 を真電源断（off→数秒→on，reflash しない）．投入後 10〜15 秒待って
   guest が hang/spin に落ち着かせる（reset ループが止まる）．
2. `esptool --chip esp32c6 --port <port1> --before default-reset --after no-reset
   read-mem 0x600B1004`（STORE1 stage）を読む．続けて `--before no-reset` で
   `0x600B1000`(synth)・`0x600B1008`(PMU DIG_POWER)・`0x600B100C`(PMU CK_POWER/
   xpd_bbpll)・`0x600B1018`(stock marker) を読む（read-mem は download mode 入りで
   LP_AON は保持）．
3. 併せて BlueZ で `ASP3-C6-BLE` の有無も（放射＝真の成功指標）．
4. 判定：
   - `STORE1=0x5A6E0004/0005`＋BlueZ 放射＝**★cold Arm B 成功**＝stock の
     完全 cold 起動を継承すれば ASP3 BT が cold ロック＝soft-reachable．次段＝
     stock 起動 bisect＋PMU dump 差分で欠落レジスタ名指し．
   - `STORE1=0x5A6E0003`（controller_enable 直前で停止）＝**guest は到達したが
     phy_init で hang**＝stock 完全起動継承でも standalone re-init は不可の傍証
     →handoff-based BT を deliverable 化 or pmu bisect（STORE3 の xpd_bbpll が
     cold で立っているか確認）．
   - `STORE1` が guest magic 無し・`STORE6=0x570C...`＝jump 到達も guest 未到達
