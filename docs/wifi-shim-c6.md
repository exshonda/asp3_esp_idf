...
**これは実測（構造体サイズの変化）で裏付けられた本物のABI不整合
バグであり，症状を直さなくても保持する価値がある**（`clk_i2c_mst_en`
と同種の「実バグだが単独ではAP0個を解消しない」ケース）．実機再
検証：修正後もログは1バイトも変わらず`wifi_scan: 0 APs found
(err=0)`のまま．コミット済み（未push）．
...
- **確定した構造的バグ1件（`CONFIG_IDF_TARGET_ESP32C6`欠如）を修正・
  コミット**（症状は不変だが独立して正しい修正）．
