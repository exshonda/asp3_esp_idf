...
14:C1:9F:E0:5A:9D`（ボードCのベースMAC`...5A:9C`+1，ESP-IDFの標準
...
- なし（`docs/wifi-shim-c6.md`への本実施83追記のみ，ソース変更
  ...
  （`/home/honda/.claude/jobs/494f98a3/tmp/idf_c6_softap/`）は
...
- ホストの`nmcli dev wifi list`で`ASP3TEST82`（`BSSID
  14:C1:9F:E0:5A:9D`，`signal=94`）を独立に確認し，コーディネータの
...
`/home/honda/.claude/jobs/494f98a3/tmp/nuttx-c6/nuttx/`）を読んだ。
その結果，2つの決定的な事実が判明した：
...
1. **ボードB同一個体のNuttX/native陽性対照が存在しない**。
   ...
   （XIAO `58:E6:C5:12:D4:D0`，実施56/65/75）のみ。現行ボードB
   （`14:C1:9F:E0:61:B0`）は実施76の登場以来ASP3専用で，NuttX/
...
`14:C1:9F:E0:61:B0`）でAPM説を検証し，凍結解除を判定する。
...
- これは実施43〜56で旧ボード（`58:E6:C5:12:D4:D0`）に間欠発生した
...
- **コード回帰は棄却**：直近のC6関連変更は3件のみで，(a) TLSコミット
  ...
  （14:C1:9F:E0:61:B0）でRESCAN 0 APsループ継続＝リグレッションなし」
...
- `ifconfig wlan0 up`成功（HWaddr=board BのMAC `14:c1:9f:e0:61:b0`）。
...
同一個体判別は**旧ボード（58:E6:C5:12:D4:D0）上で成立済み**であり，
...
7. 接触禁止6台（board C・C5#1・C5#2と各ブリッジ）は全工程で無接触
   （by-id/`adapter serial`固定，操作は`14:C1:9F:E0:61:B0`と
   `9efb0adb…`ブリッジのみ）。
