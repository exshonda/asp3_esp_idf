...
- **index 25〜43**：block=0x67（BB）のシーケンシャルなレジスタ群
  ...
  79 vs 77——一定オフセットではなく，レジスタ毎に異なる差分）．これは
  コード分岐の違いというより，**校正/実測依存の値**（ADC比較器読出し
  等，実施7で確認済みのregi2c読出しに依存する測定結果）が個体差・
  実行タイミング差で変わる自然な変動と考えられる．
...
NuttXとほぼ完全に一致し，唯一の相違点は校正ループの収束過程における
1箇所の測定依存の分岐（それ自体，コードの欠陥ではなく正常なアナログ
校正の挙動として説明がつく）のみである．
