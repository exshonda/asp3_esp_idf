...
## 【決定事項】長期方針：asp3_core共通部をFMP3同等のCLIC対応へ（2026-07-13、ユーザ決定）

本レポート判定6は「現行修正（C5チップ層のsynthetic mret）のまま」を推奨したが、
ユーザ決定により**長期方針はB案＝asp3_core共通部をFMP3と同等の設計（出口正規化型：
mret非経由出口の除去＋chip層での`mcause.mpil`処理、FMP3 `doc/clic_design.md`参照）へ
変更する**こととする。

- **理由**：ASP3/FMP3間のメンテナンスコスト低減——片方に不具合や変更があった場合、
  もう片方への対応が容易になる（同一設計なら修正の相互移植が単純化する）。
- **時期**：現行の一連の作業（C5ブリングアップ）完了後。**すぐに進める必要はない**。
- **作業の要点（着手時のメモ）**：
  - 変更先はasp3_coreリポジトリ側（本リポジトリのsubmodule禁則どおり、asp3_core側で
    実施してsubmodule bump）。FMP3の該当実装＝`core_support.S:603-616`（idle復帰）・
    `:677-689`（遅延ディスパッチ）・`esp32p4/chip_support.S:166-180`（irc_end_intの
    mpil=0強制）を参照実装とする。
  - 移行後、C5チップ層のsynthetic mret（実施28）は撤去可能になる——撤去時は
    実施28の決定実験（凍結消滅・wifiタスク起動）とtest_porting回帰の再実施で
    等価性を確認すること。本レポート§5の指摘（mnxti非互換等）はFMP3方式で解消する。
  - test_portingへの「wake-from-idle／割込みからの遅延ディスパッチ」試験追加
    （実施28申し送り・本レポート判定6-4）も同時に行うと回帰検出が効く。
