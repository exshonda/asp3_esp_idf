...
## 7. A/B 対照の試み——**「1 回失敗すると wedge して復帰しない」ことが判明し中断**

§6.4 の未確定1（`pend path engaged` は失敗の弁別指標か）を決めるため、
**成功・失敗の両方を複数回集める**必要があった。人手を減らすため
`tmp/c3_ab_trial.py` を作成（**接続前から DTR/RTS を触らず読み続け、試行を自動で
切り出して A/B/C に分類**．既存ログでの positive control 済み）。

### 7.1 得られたデータ（1 試行のみ）

```
--- 試行 #1 (+33s) ---
  分類            : B（要求来ず・ENC_CHANGE status=13）
  ★pend path      : あり
  security_initiate: rc=0
  最終 sec_state   : enc=0 bond=0
```

### 7.2 ★中断の理由＝**失敗後に広告が再開されない**

ユーザーは「1 回も接続できない・広告が消えた」と報告したが、観測プロセスは
**試行 #1 を確かに捉えていた**。つまり **1 回目の接続は DUT に届いて B で失敗し、
その後 DUT が広告を再開しなかった**ため 2 回目以降が繋がらなかった。

⇒ **A/B 対照を取るには「接続→失敗→リセット→接続」を毎回人手で繰り返す必要がある**
（1 試行ごとにリセットが要る）。本ラウンドでは割に合わないと判断し中断。

### 7.3 ★この wedge は evidence-rc-c3-P1 の wedge とは**別物**

| | evidence-rc-c3-P1 の wedge | **今回** |
|---|---|---|
| `msys_1` | 枯渇（`free=0/12`） | **健全（`free=12/12`）**（§3 実測） |
| 接続の後片付け | されない（`conn_handle` 保持） | **される（`0xffff`・`gap_disc=1`）** |
| 広告 | `g_adv_active=1` なのに電波が出ない | **広告が再開されない** |

**「接続は正しく畳めているのに、その後 advertising を張り直していない」**——
mbuf 枯渇では説明できない。**次に見るべきはここ**（§8）。

### 7.4 観測手段の事故（通算 5 件目）

集計を取ろうとして自分で `pkill -INT` を撃ち、**生ログの書き出し前にプロセスを殺した**
（`c3_ab_trial.py` は終了時に `/tmp/c3_ab_raw.log` を書く実装だった）。
⇒ **ログは «逐次 flush» で書くべき**。まとめ書きは観測プロセスの異常終了で全損する。

## 8. 次の一手（実機なしで進められる）

**「切断後に広告が再開されない」理由をコード側で特定する。** 具体的には：

- `apps/ble_host_smoke_c3/ble_host_smoke_c3.c` の `BLE_GAP_EVENT_DISCONNECT` ハンドラが
  `start_advertising()` を呼んでいるか、**その戻り値をどう扱っているか**
  （rc を捨てていれば「失敗しても黙って進む」＝今回の症状と整合する）。
- `start_advertising()` 内の `ble_gap_adv_start()` の rc ログ（`adv rc=` マーカ）が
  失敗経過で出ているか。

これは **read-only のコード調査**で進むため、実機・スマホ操作は不要。
特定できてから、必要なら 1 回だけ実機で確認する。

## 9. 本ラウンドの確定事項（まとめ）

**解決した**：
- SM が常に無効化されていた cmake バグ（evidence-05 §3）→ **BlueZ で bond 成立**
- bond 永続化（フラッシュ→**真cold**→再ペアリング不要で復元．evidence-05 §4）
- `esp_rom_spiflash_config_param` が Direct Boot に必要（evidence-04 §7.1）

**実測で確定した事実**：
- ペアリングは**空中で完全成功**する経過がある（鍵配布双方向．§2）
- 切断は**ホストに正しく配送**される（§3．fable の中心仮説を反証）
- Android 相手の挙動は**少なくとも 3 通りに分岐**する（§6.1）＝単一の決定論的バグではない
- 失敗後は**広告が再開されず、リセットするまで復帰しない**（§7.2）＝mbuf 枯渇とは別機構（§7.3）

**未確定**：
- `pend path engaged` が失敗の弁別指標か（**A/B 対照が未完**）
- 経過 A の `PAIRING_COMPLETE` 時点で `encrypted=1` か（ログは仕込み済み・**未捕獲**）
- **なぜ切断後に広告が再開しないか**（→ §8）
