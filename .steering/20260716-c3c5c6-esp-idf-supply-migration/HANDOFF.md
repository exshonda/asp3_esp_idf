...
| **C5 BT 供給移行** | **⚠️ビルドは復旧・実機検証が未完**（下記 §4-1） |
...
証跡＝`.steering/20260716-c3c5c6-esp-idf-supply-migration/evidence-c5-01〜05`。**evidence-05 §6 は記録未完**（親がサブエージェントを停止したため）。
...
- **C3 BT は hal 版のまま**（v5.5.4 切替は実機 bond 失敗＝`AuthenticationTimeout` のため既定 OFF で維持）。
...
- **C5#2**：BASE MAC **`d0:cf:13:f0:c8:94`**、native USB-JTAG=`ttyACM5`
...
- **Wi-Fi 認証情報**：**ビルド注入のみ**（`CMAKE_C_FLAGS` 経由。**`-DWIFI_SSID=` は cmake に直接渡しても効かない**＝
  ...
  （`wifi_connect.c` は SSID を syslog に出す＝evidence 化時はマスク）。**commit 前に `git diff` で混入確認**。
  ★**訂正（2026-07-17）：「現在まで混入 0 件を維持」は事実ではなかった**——実測の結果、**SSID が
  13 commit・6 リモートブランチに混入し、public リポジトリへ push 済み**だった（`docs/c5-bringup.md`
  `docs/ble-c5c6.md` `docs/wifi-shim-c6.md` `docs/load-test-c3c5c6.md` `tmp/c6_step0_findings.md` と
  memory 2件、計33箇所）。**現 HEAD と memory からは除去済み**（`<SSID-2G>`/`<SSID-5G>` にマスク）だが
  **履歴には残る**（public repo の履歴書換えは force-push が要るうえ GitHub が orphan commit を SHA 直参照で
  保持し続けるため不完全＝ユーザー判断でここでは実施せず）。**パスワードの混入は 0 件**（履歴含め実測確認）。
  ⇒ **この種の「〜を維持」という自己申告を検算せずに信じるな**（HANDOFF §5-10 の実例が §6 自身に在った）。
  **AP 環境は変わりうる**（前回は `…_A2`/ch9 だった）。**まず `wifi_scan` で実在する SSID を確認してから W1 を組む**。`reason=201` を回帰と誤断しない。
...
| option | 既定 | 意味 |
|---|---|---|
| `ASP3_ESPIDF_SUPPLY` | **ON** | esp-idf submodule 供給（OFF で hal 供給へ完全復帰） |
