...
### 9.3 ★★C6 の真POR は LP_AON を消す（sentinel で実証・本ラウンド初）

**C5 は §7.1 相当で実証済みだったが、C6 では未検証だった**ので測った。

| 手順 | 実測 |
|---|---|
| 1. STORE6 に sentinel `0xCAFE5A9C` を植える（download mode） | 読み戻し **`0xcafe5a9c`**＝**植込み成功** |
| 2. **真cold**（`-p 2 off`＋by-id 2ノード **GONE** を読み戻し） | — |
| 3. POR 後に STORE6 を読む | **`0x00000000`** |

⇒ **C6 の真POR は LP_AON をクリアする**（C5 と同じ）。

★**この結論が over-determined でないことを先に確認した**（＝«アプリが boot で 0 クリアしている» 可能性の排除）：
**app の全 `sil_wrw_mem` を列挙**し、**STORE6 への書込みは `on_reset()`（tag `0x5E00`）と
ENC イベント（tag `0x5DE0`）の2箇所だけ＝boot 経路には無い**ことを実測。
boot 経路（`:742-758`）が触るのは **STORE0/1/2/3（診断スナップショット＋sync/adv）と
STORE8/9（CONN/DISC を 0 クリア）のみ**。
⇒ **STORE6 の 0 は «アプリの仕業» ではありえず、POR の仕業**（かつ `on_reset` が発火していれば
`0x5E00xxxx` が見えるはずで、0 ではない）。**⇒ sentinel テストは valid。**

★★**含意（Android セルへ遡及）**：**Android セルの前に真cold を実施している**
⇒ **あのセルのマーカは «原理的に» フレッシュ**だった（POR が消すので stale はありえない）
⇒ **`0x5dc00011` が «古い bond の再利用» である可能性は二重に否定される**
（(i) タグ `0x5DC0` が立った＝新規 PAIRING が発火した、(ii) POR が LP_AON を消していた）。

### 9.4 セル準備完了（★ユーザー確認待ち＝ここで停止）

| 項目 | 実測 |
|---|---|
| 載っているビルド | **`build/gd_c6_ble`＝アーム B**（**再ビルドせず**・`verify-flash` **digest matched**・§9.1） |
| **真cold** | **`usbjtag=GONE cp2102n=GONE`** を読み戻し（★`uhubctl` は segfault するが電源は切れる＝読み戻しだけが証拠） |
| **マーカー** | ★**下表（タグで確認）**。Android セルの値は**消えている** |
| BlueZ central の stale bond | **0 件**（`RemoveDevice` 実施） |
| **広告在席（スキャンで確認）** | ★**`name=ASP3-C6-BLE RSSI=-71 Paired=0`**（**esptool は使っていない**） |

**真cold 直後のマーカー実測（★«0» ではなく «タグ» で読む）**：

| reg | 実測値 | 判定 |
|---|---|---|
| **`0x600B1018`（ENC）** | **`0x00000000`** | **クリア**（Android の `0x5de00000` は消滅） |
| **`0x600B101C`（PAIR）** | **`0xa1020704`** | ★★**«0» ではないが «クリア»**＝**タグ `0xA1`＝bt_shim の intr トレース（controller init 時の書込み）**。**`0x5DC0` ではない＝PAIRING 未発火** |
| `0x600B1020`（CONN） | `0x00000000` | クリア（app が boot で 0 クリア） |
| `0x600B1024`（DISC） | `0x00000000` | クリア（同上） |
| `0x600B1014`（intr 線2） | `0x00000129`＝**297** | フレッシュ（Android セル後は `0x5c25`＝23589 だった） |

★★**«アドレスでなくタグで引け» が実際に効いた瞬間**：**PAIR は `0x00000000` ではない**。
**«値が 0 なら清浄» という基準で読んでいたら «クリアに失敗した» と誤断**していた。
**正しい基準＝«タグが `0x5DC0` でないこと»**。
**同時に、app のコメント «bt_shim は controller init 時のみ書込み» が実測で裏付けられた**
（init 値 `0xa1020704` がここに居る＝**PAIRING が発火すれば `0x5DC0` で上書きされる**）。

**★ユーザーへ渡す手順（親経由）**

1. ★★**両方のスマホで forget → BT OFF/ON**。
   **Android は数分前に C6 と bond して鍵を持っている**が、**デバイス側 bond store は RAM-backed で
   真cold により鍵を失った**（§9.3 で POR が LP_AON を消すことを実証）
   ⇒ **«スマホは鍵を持つ／デバイスは失った» の不一致**＝**Android の自動再接続が本セルを汚す**。
   **★1ビルド1端末**（`ENC` は最後の1件しか保持しない）。
2. 接続 → **`0xABF1`＝`BT4-OK`** → **`0xABF2`** subscribe → **`0xABF3`** WRITE →
   **`0xABF4` は未ペアで弾かれるのが正しい** → ペアリング → **bond 後に `BT4-OK` なら D-2d 達成**。
3. ★**«最初の disconnect でエラー» は既知の正常挙動**（デバイス側 reason `0x13`＝正常切断）。
   **回帰ではない。**

★**ユーザーが「終わった」と言うまでマーカーを読まない**（`esptool` は download mode へ落として広告を止める）。

---

## 10. スマホ セル2 ＝ **B × iPhone** — 実測（★OK）

**ユーザー観測（逐語）**：「**iPhone x C6 : all ok**」

### 10.1 マーカー実測（★タグで引く）

| reg | 生値 | タグ | 意味 |
|---|---|---|---|
| **`0x600B1018`** | **`0x5de00000`** | **`0x5DE0`** | **ENC_CHANGE・status=`0x00`＝暗号確立**（reset_cb `0x5E00` ではない＝ble_hs リセット無し） |
| **`0x600B101C`** | **`0x5dc00011`** | **`0x5DC0`** | ★★**PAIRING_COMPLETE・status=`0x00`・our_sec=1・peer_sec=1＝DUT 側 bond 成立** |
| `0x600B1020` | `0x604e0002` | `0x604E` | CONNECT・status=`0x00`・count=2 |
| `0x600B1024` | `0xd15c1302` | `0xD15C` | DISCONNECT・reason=`0x13`・count=2＝**正常切断**（remote user terminated） |
| `0x600B100C` | `0xad000000` | `0xAD00` | adv rc=0。★**`0x7717` 不在は判定不能**（§5.2）＝**判定に使わない** |

デコードは **ソースの式**（`:615-620`）による（memory の C3/C5 形式に頼らない）。

### 10.2 ★★「前状態→後状態」の遷移が直接証拠になった

**準備時（§9.4）に PAIR レジスタの «前» を測って commit しておいた**ため、
本セルは «`0x5DC0` が在る» ではなく **«`0x5DC0` が既知の前値を置き換えた»** という
**同一レジスタ・同一セッション内の遷移**として示せる：

| reg | 準備時（真cold 直後・`b3e131b` で commit 済） | iPhone セル後 | 遷移 |
|---|---|---|---|
| **PAIR `0x600B101C`** | **`0xa1020704`**（bt_shim init・タグ `0xA1`） | **`0x5dc00011`** | ★**`0xA1` → `0x5DC0`＝本物のペアリングが発火した直接証拠** |
| **ENC `0x600B1018`** | `0x00000000` | `0x5de00000` | 0 → 暗号確立 |
| CONN / DISC | `0` / `0` | `0x604e0002` / `0xd15c1302` | 0 → 2接続2切断 |
| intr 線2 `0x600B1014` | `0x00000129`（297） | `0x00007031`（28721） | **+28,424＝アプリが実セッションを走った** |

★**stale 読みの否定（重要）**：**Android セルと iPhone セルの値は完全に一致している**
（`0x5de00000` / `0x5dc00011` / `0x604e0002` / `0xd15c1302`）。
**«Android の残値を読んでいるだけ» の可能性を潰す必要がある**——潰せる：
**準備時に «Android の値が消えている» ことを読み戻して commit してある**（§9.4）
⇒ **現在の値はすべて準備時の読み戻しより «後» に書かれた＝iPhone セルのもの**。
**⇒ 準備段階の «クリア＋読み戻し検証» が、この判定を可能にした唯一の手順。**
（値が一致するのは **両セルとも同じ形で成功したから**＝接続→切断→再接続→切断・status=0・our=1/peer=1。
★**«最初の disconnect でエラー» は既知の正常挙動**＝reason `0x13`＝**回帰ではない**。）

★**二重のフレッシュ性**：(i) 両セルとも **真cold が先行**、(ii) **真POR が LP_AON を消すことを
sentinel で実証済み**（§9.3・over-determination も排除済）⇒ **両セルのマーカは必然的にフレッシュ**。
...
| **P1** | B × W1 が通る **88%** | **★的中**（B 2/2 PASS・timeout 0） |
| **P2** | B × W2(BlueZ) が通る **93%** | **★的中**（B 2/2 PASS） |
| **P3** | **B × iPhone が OK ＝94%**（`5ad34cd` で測定前に commit） | **★★的中** |
| **P4** | B × Android が OK を維持 **93%** | **★★的中** |

★**P3 の改訂経緯を残す**：**当初 90%**（§3.3・C6 実機に触る前）→ **本セル直前に 94% へ改訂**
（`5ad34cd`）。**改訂根拠は測定«前»に出揃った情報のみ**＝(1) B は A2 とバイト同一、
(2) 同じ bin で BlueZ 4/4、(3) 同じ bin で Android が同日 OK、(4) 既知マトリクス。
**後付けで数字を動かしていない**（改訂は測定前・commit 済み）。

### 10.4 ★★言えること／言えないこと（射程を守る）

**言えること（実測のみ）**：
- ★★**C6 は Android と iPhone の «両端末» で、«今日»・«フレッシュに»・bond 成立を実測で実証した**
  （両セルとも `0x5DC0` status=0・our_sec=1・peer_sec=1、かつ ENC status=0）。
  **プロジェクトで唯一の «両方通るチップ» が、実測で固まった。**
- **`our_sec=1`＋`peer_sec=1`** は本プロジェクトで稀（memory：C3 hal×Android が初）。
  **C5 の判定基準「status=0 かつ our_sec>=1」は C5 では一度も満たされた記録が無いが、
  C6 は «両端末で» 満たした。**

**★言ってはいけないこと（本ラウンドでは未特定）**：
- ★**«なぜ C3/C5 が落ちて C6 が通るのか» の機序は言えない**。**チップ差の «事実» は言えるが
  «機序» は未特定**（本ラウンドは機序を測っていない）。
- ★★**«toolchain が Android/iPhone を通した» とは書けない＝手柄にしない**。
  **B は既知良好 A2 とバイト同一**（135/135・差3バイト＝`__TIME__`）
  ⇒ **toolchain は機械語を何も変えていない**⇒ **通ったのは «元から通っていたから»**。
  **本ラウンドが示したのは «IDF 標準 toolchain へ揃えても壊れない» までであって、
  «良くなった» ではない。**
- ⇒ **収穫の正確な形**＝**Android/iPhone 調査から toolchain を «除外» できる**（帰属の前進）。
...
| **W1（DHCP+ping）真cold** | **A 2/2 PASS（51/0・50/0）／B 2/2 PASS（50/0・49/0）**・timeout 全 run **0** |
| **W2 BlueZ 真cold** | **A2 2/2 PASS／B 2/2 PASS**（`0xABF1`=`BT4-OK`・`0xABF3` WRITE OK） |
| **W2 Android（アーム B）** | ★★**OK**（`ENC=0x5de00000`／`PAIR=0x5dc00011`＝status 0・our=1・peer=1） |
| **W2 iPhone（アーム B）** | ★★**OK**（同上。**`0xA1`→`0x5DC0` の遷移で直接実証**） |
| **BLE の A2 vs B** | ★**縮退＝135/135 オブジェクト同一・差3バイト＝`__TIME__` のみ**⇒**B は既知良好そのもの** |
| **march** | ★**変更不要＝`asp3_core` 変更不要**（135/135 同一・atomic 0・`Tag_RISCV_arch` は実際に変化＝実験は有効だった） |
| **toolchain guard** | **`target.cmake` 2行で転写・FATAL 6/6 実発火・退避先の実在確認済み** |
| **`esp_timer.h` shadow** | **根治（3チップ共有）**。C6 は元々無傷（`bt_esp_timer_ext.h` で回避済み） |
| ★**真POR は LP_AON を消す** | **C6 で初実証**（sentinel `0xCAFE5A9C`→`0x00000000`・over-determination 排除済） |

## 12. ★未測定（最終・隠さない）

- ★**`__TIME__` バナーを «走行ブート» で取り逃した**（全 cold run）。
  **`verify-flash` は «保存イメージ» の digest であって «走行イメージ» の観測ではない**
  ⇒ **H2 の «載っているビルドの独立同定» は完全には閉じていない**。
- **B2（コンパイラ B × ISA rv32imac）の実機 run＝未実施**（機械語 no-op と実証済＝差が出る余地は無いが、
  **反証の機会を作っていない**）。
- **C3/C5 の実機非回帰＝未実施**（Phase 1 の非回帰は**ビルド実測のみ**。★**«builds» ≠ «known-good»**）。
- **`0xABF3`（`0x7717`）の発火可否＝構造的に判定不能**（両セルとも）。
- **`0xABF1` にマーカ無し**（BlueZ 側で `BT4-OK` 4/4 実測済み）。
- **C3/C5 が落ちて C6 が通る機序＝未特定**（§10.4）。

## 13. 申し送り（★C3 フェーズの入力）

1. ★**`heap_caps_malloc` は hal 側＝シムが要る**＝**C3 最後のフラグ依存**。
   `hal/components/esp_phy/src/phy_init.c:470` が `esp_heap_caps.h` を include せずに呼ぶ
   （esp-idf 版は `freertos/FreeRTOS.h`/`esp_check.h` 経由で推移的に得るが hal 版はその世代でない）。
   **hal は submodule＝編集禁止**⇒ **force-include か shim**。
   **これを潰せば C3 も `-Wno-error=implicit-function-declaration` から解放される**（3件→1件→0件）。
2. **`esp_timer.h` の残り10穴**。うち **2本は `int64_t` 戻り**
   （`esp_timer_get_next_alarm{,_for_wake_up}`）＝**暗黙 int だと上位32bit を黙って捨てる**。
   **現状の呼び出しは 0 件＝潜在**（検出器較正済み）。
3. **`endian.h` の shadow**（本物＝`esp-idf/components/bt/porting/include/os/endian.h`・識別子 49）。
4. **`esp_wifi_v8.cmake:92-97` の4フラグ抑制はもう何も隠していない**（較正済み実測 0/0/0/0）
   ⇒ 撤去可能と思われる。
5. **`ble_host_smoke_c6.c:112` の STORE4 衝突は実在**（`rom/rtc.h:62`＝`RTC_XTAL_FREQ_REG = LP_AON_STORE4_REG`）。
   **現状無害**（観測された `invalid RTC_XTAL_FREQ_REG` 警告は STORE4 を書かない `wifi_dhcp` でも出る＝pre-existing）。
   **ただし STORE4 を «信頼できる割込みミラー» として読むな。**
6. **C6 の `bt/bt_esp_timer_ext.h` は本修正で冗長化**（同一宣言＝合法・実測で衝突なし）。**残置**。
7. ★**guard 転写は C3 も «2行»**（`asp3/cmake/esp_toolchain_check.cmake` はチップ非依存・
   `target.cmake` は本リポジトリ＝**submodule 変更不要**）。
   ★**ただし C3 に入れると «既定 toolchain が esp-15.2.0 の現行 C3 レシピ» が FATAL になる**
   ＝**挙動変更**なので、**入れる前に C3 の既知良好レシピをどう扱うか（`ASP3_ESP_EXPECTED_TOOLCHAIN` で
   受け入れるか・版を揃えるか）をユーザーに確認すべき**。
8. ★**C3 の march は IDF 標準と «既に一致»**（両方 `rv32imc_zicsr_zifencei`）
   ⇒ **C3 では march 論点そのものが発生しない**（`soc/project_include.cmake:8-10` 実測）。
