...
by-id（`<MAC-37>`）へ `--chip esp32c5`・`Hash of data verified`。
...
**全 3 接続とも peer=`<MAC-37>`（DUT・public）＝ ★H-mixup は snoop でも死んだ。**
...
### 9.5 実験 2 の結果＝**成功（ユーザー逐語「Android : all ok」）・P_2/P_2sig 的中**

**マーカー実測（00:44:37・実験 1 と分離済のブート・snoop 無し＝カウンタが唯一の計器）**：

| reg | 値 | 読み |
|---|---|---|
| ★STORE4 | ★**`0xe2060608`** | **RX=6・L2=6・TX=8＝3 セル連続で同型**（P_2sig 的中） |
| STORE3 | `0x3302000a` | ★**enc_chg=3・sm_enc_rx=3**＝暗号化 3 回（ペアリング 1＋**LTK 再接続 2**）・最終 enc 以降 put=2/sm_tx=0/to_ll=10（SMP 無し＝再接続は鍵再利用） |
| STORE6 | `0x5de00000` | ENC status=0 |
| STORE8 / STORE9 | `0x604e0003` / `0xd15c1303` | CONN=3・**DISC=3（reason 0x13）**。★**DISC==CONN＝読取時点で接続なし＝今回の read-mem はリンクを落としていない**（毎回確認の注記・前回 §8.4 との対比） |
...
| 候補 | 生死 |
|---|---|
| **②（snoop ON の観測者効果）** | ★**必要条件として死亡**（snoop 無しで成功） |
| **①（BT スタック再起動が直す）** | 生存。★**ただし今回の設計では «sticky 状態» と分離できない**（BT OFF/ON を再適用したため。「①の再適用が効いた」と「一度直った状態が持続している」を区別する情報が無い）＝**断定しない** |
| **sticky 状態（一度直ると持続）** | 生存（同上・①と未分離） |
| **⑤（元々間欠）** | 生存。★**殺すには «失敗の再現» しかない**＝実験 4 の存在意義 |
| ③時刻/RF・④私の清掃/verify | 弱いが形式的に未検定のまま（この 3 連続成功と両立しにくい） |

---

## 10. 判断表（★実施しない・ユーザーの選択・推奨に印）

**現状**：**成功 3/3**（全て真cold・forget 後フレッシュ・両側 or デバイス計器で 6/6/8 一致）・
**全事前登録的中（枠外セルを除く）**・★**ユーザー要件「Android が繋がる」は現に満たされている**。

| 選択肢 | 何が分かるか（判別力） | 費用 | リスク | 推奨 |
|---|---|---|---|---|
| **実験 3（スマホ再起動→再試行）** | 低：再起動＝スタック再起動の強い形＝おそらく成功のまま。①/sticky/⑤ どれも殺せない | スマホ操作のみ | ほぼ無し | — |
| **実験 4（長時間運用で失敗の再現待ち）** | ★**最大＝機序確定の唯一の道**：失敗が snoop ON で撮れれば (i)/(ii)/(iii) が直接判別（§7.9 の A/B 成立）・⑤も決着 | ★**読めない**（再現条件が不明＝日単位の可能性） | 待ち時間が無駄になる可能性 | ○（機序が要るなら） |
| ★**ここで閉じる** | 機序は **«未決» と記録して閉じる**。**成果物＝「再発したら即座に撮れる体制」**（E-RX ビルド＋両側計器＋本 evidence の手順一式。再発時は snoop ON→再現→bugreport→§7 の型で両側観測を再開できる） | ゼロ | **再発リスクは残る**（機序未特定のため予防はできない）。ただし検出・診断手段は整備済 | ★**推奨**（要件は満たされている。機序解明は保険であり、その判断はユーザーのもの） |
| **実験 4 を «ON のまま放置» で兼ねる** | 閉じつつ、★**snoop を ON に戻して**日常使用＝再発した瞬間に撮れる（実験 4 の受動形） | snoop の常時 ON（端末負荷・ログ肥大） | ログローテートで失敗時刻を逃す可能性 | ○（折衷） |

★**実験 4 をやるなら手順に必ず「snoop を ON に戻してから待つ」を含める**（失敗が出た時に撮れる体制）。

**次フェーズの選択肢（1 行）**：★**本ラウンドで確立した «デバイス側カウンタ＋phone snoop の両側観測» の型は、
C3 の残り 2 軸（B: Android 再接続不可・C: iPhone wedge）へそのまま転写できる**（C3 は STORE レイアウト差のみ）。

## 11. 最終セル＝C5 × iPhone（回帰確認・ユーザー決定「閉じる。iPhone で最後テスト」）＝ **準備完了**

### 11.1 目的と事前登録（★実施前 commit・書き換えない）

**目的**：iPhone（歴代 OK の健全セル）が **«今の状態でも» OK のままか**＝回帰確認。

| # | 予測 | 登録値 |
|---|---|---|
| **P_iph** | **iPhone のペアリングが成功する** | **90%** |
| P_iphsig | 成功時カウンタ：tag=0xE2・ENC=`0x5de00000`・RX∈[4..6]・L2==RX・TX∈[5..8]（iPhone の鍵配布数は未実測＝Android より緩い枠。**iPhone のカウンタ既往値は存在しない＝今回が初の基準値**） | 80%（成功条件下） |

★**外れ（失敗）の意味＝重大**：**今回の一連の状態変化（スマホ側 or 検証操作）が «歴代 OK の iPhone セル» を
壊したことになる**＝閉じずに要調査。**成功なら回帰なし＝閉じる**。
...
|---|---|
| ビルド | 焼き直し無し＝`verify-flash` digest matched |
| マーカー | STORE0,2-9 クリア→ALL ZERO 読み戻し（実験 2 と分離） |
| 1セル1ボード | `Port 1/2: 0000 off` 読み戻し＋C3/C6 by-id GONE＋スキャンで ASP3-C5-BLE のみ在空（較正 7 デバイス） |
| 真cold | `jtag=GONE cp2102n=GONE` 読み戻し→復電→広告実測 ⇒ デバイス側の鍵ゼロ |
| BlueZ | device remove・Discovering: no |

### 11.3 ユーザー手順（★順序が肝）

1. ★★**先に Android で `ASP3-C5-BLE` を forget**（**Android は直前の bond の鍵を持っている**。
   デバイスは真cold で鍵ゼロ＝**Android の自動再接続が iPhone セルを汚す**）。
2. **iPhone の Bluetooth 設定（または既往の接続アプリ手順）で `ASP3-C5-BLE` をペアリング**。
   （iPhone に古い `ASP3-C5-BLE` 登録が残っていれば先に「このデバイスの登録を解除」）
3. **観測を逐語で報告**：ペアリング要求→結果・登録の有無・（失敗なら）タイミング感。

### 11.4 結果＝**成功（P_iph 90%・P_iphsig 80% とも的中）＝回帰なし**

**ユーザー観測（逐語）**：「**iPhone : all ok(以前と同様にたまに切断時にタイムアウトする）**」
★**「切断時のタイムアウト」は既知の正常挙動**（stock でも発生を実測済＝`evidence-stock-01 §10`・
ユーザーが以前「このままで良い」と判断済）＝**回帰ではない**。

**マーカー実測（00:52:54）**：

| reg | 値 | 読み |
|---|---|---|
| ★STORE4 | ★**`0xe2060608`** | **RX=6・L2=6・TX=8＝Android と同一の型**（iPhone も Identity 鍵を双方向配布）。登録枠 RX∈[4..6]/TX∈[5..8] 内＝P_iphsig 的中。**iPhone の初の基準値として記録** |
| STORE3 | `0x3309000c` | enc_chg=3・sm_enc_rx=3（ペアリング 1＋LTK 再接続 2）・最終 enc 以降 put=9/sm_tx=0/to_ll=12 |
| STORE6 | `0x5de00000` | ENC status=0 |
| STORE8 / STORE9 | `0x604e0003` / `0xd15c1303` | CONN=3・DISC=3（reason=0x13 最終値）。**DISC==CONN＝read はリンクを落としていない**（毎回注記） |
...
- **成功 4/4**：Android フレッシュ bond ×3（枠外 1＋実験 1・2）＋ **iPhone 回帰 1**。
- **C5 は iPhone / Android / BlueZ の 3 セントラル全てで bond 成立**（+LTK 再接続も Android/iPhone で実測）。
- **全予測の成績（★登録の書き換えゼロ）**：

| 登録 | 値 | 結果 |
|---|---|---|
| P_C0 80%／P_TAG 90%／P_CAL 85%／P_BASE 85% | — | 全的中 |
| R0-R4（失敗セル分岐） | R3=60% 本命 | ★**R4（5%）が起きた＝本命外れ**（正直に記録済） |
| S1-S3（snoop 分岐） | — | **枠外**（前提の失敗が消えた＝適用不能と記録） |
| P_repro 70%／P_sig 75%／P_2 75%／P_2sig 75%／P_iph 90%／P_iphsig 80% | — | 全的中 |

### 13.2 仮説の最終生死＝**機序«未決»のまま閉じる**

| 候補 | 最終判定 |
|---|---|
| ②snoop 観測者効果 | **死亡**（snoop 無しで成功） |
| ①BT スタック再起動が直す ／ sticky 状態 | **生存・相互に分離不能**（分離には «BT OFF/ON 無しの失敗再現» が要る） |
| ⑤元々間欠 | **未決**（失敗の再現でのみ決着可能） |
| 失敗時の (i)電波/LL／(ii)controller→host／(iii)phone 不送出 | **未決**（snoop ON での失敗再現待ち） |

★「即エラー」（ユーザー体感）vs「≥5 秒生存」（SecReq 発火の構造的事実）の**矛盾も未解決のまま保持**。

### 13.3 ★「何が変わったのか？」への公式回答（ユーザーの問い・正直に）

★**訂正の経緯を残す**：親の初版指示は「最有力＝forget では消えず «スタック再起動» で消える状態」だったが、
**検算**（本ファイル §5.2 line 258）で **E-RX 失敗セルの手順自体に「両端末 BT OFF→ON」が含まれていた**
ことを確認 → **«単なる BT OFF/ON が直す» なら失敗時代に直っていたはず＝言い過ぎ**として親自身が修正、
本節はその修正版である。★検算の粒度も正直に：**OFF/ON の記載が確認できたのは E-RX 失敗セル手順のみ**
（E1=c5-09 §8.5 の手順は forget のみ・c5-08 にも記載を確認できず）。
★**「手順に書いた」≠「ユーザーが毎回実施した」＝コンプライアンスは両時代とも検証不能**
（断定の禁止は両方向に効く）。

1. **我々の側（デバイス・ファーム・ビルド・個体・手順）は何も変わっていない**：
   失敗セルと成功セルは **flash digest 一致（verify-flash で毎セル実証）・同一個体（MAC 同定）・
   双方真cold（by-id 消滅読み戻し）・同一の 1セル1ボード統制**。**変わったのはスマホ側**。
2. ★**一回性のトリガは特定できない**：境界（失敗 00:00 前 → 成功 00:16 以降）で起きたスマホ側の変化は
   **«snoop 有効化を伴う» BT 再起動**（通常の OFF/ON とスタック構成が変わりうる）・**開発者モード有効化**・
   **自然回復**のいずれか。**«単なる BT OFF/ON» は失敗時代にも（少なくとも手順上）行われていた＝
   説明として弱い**。snoop ON «そのもの» の必要性は実験 2 で除外済み。
   **以後の状態は sticky**（成功 4/4・snoop OFF に戻しても維持）。
3. ★**軸 A の精密な言い方**：**「我々の統合は、stock が耐えられるスマホ側の状態に対して «脆い»」**。
   stock 対照（07-17・`evidence-stock-01`）は**同じ «悪い状態» の時代**に stock OK / 我々 NG を示した＝
   **帰属の対照は生きている**。**ハードな全損ではなく «感受性の差»**。
   その機序（**なぜ stock は耐え、我々は SMP を 1 個ももらえないのか**）が、将来の再発時に追うべき問い。
4. ★**歴代の NG 測定は誤測ではない**：失敗時、**較正済みの計器**（BlueZ 成功セルで 4/4/7、Android 成功
   セルで 6/6/8 を数えた同じカウンタ）が「**対向の SMP が 1 個も来ていない**」を示していた＝
   **スマホが本当に（我々の host まで）送ってきていなかった**。2アーム9レジスタ・E1・E-RX 失敗セルの
   測定はすべて**その時点の真の病態**を記録している。

### 13.4 ★再発時の手順書（＝閉じるときの成果物・両側観測を数分で再開できる）

1. **デバイス側（E-RX ビルド）**：
   - ビルド：`build/erx_c5_rxtrace`（本 evidence §3.2 の構成）。無ければ cmake 再構成：
     `cmake -S asp3/asp3_core -B build/erx_c5_rxtrace -G Ninja -DCMAKE_TOOLCHAIN_FILE=$PWD/asp3/cmake/toolchain-esp32-riscv32.cmake -DASP3_TARGET=esp32c5_espidf -DASP3_TARGET_DIR=$PWD/asp3/target/esp32c5_espidf -DASP3_APPLDIR=$PWD/apps/ble_host_smoke_c5 -DASP3_APPLNAME=ble_host_smoke_c5 -DESP32C5_BT=ON -DESP32C5_BT_SM=ON -DESP32C5_BT_RXTRACE=ON -DESP32C5_BT_PEND_DIAG=OFF -DESP32C5_IDF_STD_ISA=ON -DASP3_ESPIDF_SUPPLY=ON -DASP3_BT_IDF_V554=ON`
     （toolchain PATH＝`~/.espressif/tools/riscv32-esp-elf/esp-14.2.0_20260121/.../bin`）
   - 焼き：`esptool --chip esp32c5 --port <C5 の by-id> --before usb-reset --after watchdog-reset write-flash 0x0 build/erx_c5_rxtrace/asp_flash.bin`
   - マーカー（esptool read-mem・★読むとリンクが落ちる＝セッション終了後に読む）：
     **STORE4 `0x600B1010`＝`0xE2|smp_rx_put|smp_rx_l2|smp_tx_ll`**（タグ 0xE2 必須）・
     STORE3＝RXTRACE 原パック（enc ゲート）・STORE6＝`0x5DE0|delta|status`・STORE8/9＝CONN/DISC。
     健全基準値：**BlueZ 4/4/7・Android 6/6/8・iPhone 6/6/8**。失敗署名：**0/0/1×接続数＋ENC=07**。
   - セルの型：マーカークリア読み戻し→真cold（by-id 消滅読み戻し）→1セル1ボード（他ボード電源断＋
     スキャン不在実測）。
2. **スマホ側（snoop）**：開発者向けオプション→HCI スヌープログ ON→**BT OFF/ON**→再現→
   **BT を触らず** USB 接続→`adb bugreport <name>.zip`→zip 内 `FS/data/log/bt/btsnoop_hci.log`→
   `scratchpad の btsnoop_parse.py`（datalink 1002/H4。**Enc Change は v2=0x59**）。
   ★**再現操作より先に snoop ON にしておくこと**（今回、失敗セッションは撮り損ねた）。
3. **突き合わせの型**（§7.8）：SMP 本数（方向別）・enc 回数・接続数・切断 reason（phone 0x16 ⇔ device 0x13）。
4. ★注意：**失敗→成功の境界はスマホ側の状態遷移と一致した**（トリガは §13.3-2 のとおり特定できず。
   «単なる BT OFF/ON» では説明が弱い）＝**失敗が出たら、スマホ側の設定・再起動等の «復旧操作をする前に»
   snoop 付きの再現を撮る**（撮り損ねると今回同様、機序が閉じない）。

### 13.5 次フェーズの選択肢（1 行）

**本ラウンドの両側観測の型（デバイス側 SMP カウンタ＋phone snoop）は、C3 の 2 軸
（B: Android 再接続不可・C: iPhone wedge）へそのまま転写できる**（C3 は STORE レイアウト差のみ）。

## 12. 本ラウンド（c5-10）の最終サマリ

| 項目 | 結果 |
|---|---|
| E-RX 本来の問い（失敗時に SMP は流れているか） | ★**答えた**：失敗時＝SecReq 1 本/接続のみ・対向 SMP 0（R4）。成功時＝6/6/8 のフル交換 |
| 計器 | wrap 6/6 逆asm 証明・BlueZ/Android/snoop の 3 系で較正・**phone 側独立記録と本数一致 3 セル** |
| 病態の現状 | ★**「C5×Android bond 不成立」は現在の状態では再現しない（最終 4/4＝Android 3＋iPhone 回帰 1・§13）**。境界＝BT スタック再起動前後・**因果未確定** |
| 機序 | ★**未決**（(i)/(ii)/(iii) 判別は失敗の snoop 付き再現待ち）。「即」vs「≥5 秒生存」の緊張も**未解決のまま保持** |
| 事前登録 | R4 的中（低確率側）・R3 外れ・C0/P_* 系は全的中・**登録の書き換えゼロ** |
| 禁則 | 修正コード 0・submodule 不触・hub 1-5 不触・認証情報 0 |
