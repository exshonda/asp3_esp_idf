...
**事故**：本ファイル初版（commit `9c27cd6`）の §5-3 で、**「混入0を確認する grep の «式» そのもの」に
SSID リテラルと パスワード断片2つ（`<X>` 以外の2片）を書いてしまい、push した**。
＝**漏洩防止手順を記録する節が漏洩源になった**（3行・SSID×2 と PW断片×2）。
...
**是正**：`264fb99`（コーディネータの masking）と**同じ fix-forward 方式**で HEAD をマスクした。
...
6. **★★私が commit `9c27cd6` で認証情報を混入させた（§5.1）＝ユーザー判断が要る**：
   - **内容**：SSID リテラル×2行・**パスワード断片×2**（`<X>…` の2片．`<X>` を含む完全形ではない）。
     ＝コーディネータが `264fb99` で確立した「SSID 混入 0」を**私が一時的に回帰させ**、
     さらに「**パスワード混入 0 件**を維持しろ」という指示に対しても**断片レベルで違反**した。
   ...
   - **★被害範囲の «実測»**（`git log --all -S<lit>` で数えた．推測ではない）：
     ...
     | 対象 | 含む commit | 評価 |
     |---|---|---|
     | **パスワード完全形**（`<X>` まで含む文字列） | **0 commit** | **★一度も commit されていない＝「PW混入0」は «完全形» については維持** |
     | **パスワード断片2片** | **2 commit＝`9c27cd6`(私の漏洩)・`5f1284e`(私の是正)＝両方とも私** | **★私が «0 だったもの» を «2» にした**。断片は完全形から `<X>` を欠くのみ＝**部分開示として実質的** |
   ...
   - **判断事項**：(i) 履歴書換え（filter-repo/rebase + force push）を行うか——
     **別エージェントが同一ブランチで同時に commit 中のため私の判断では実行しない**。
     ...
     (ii) **当該 AP のパスワード変更**——**完全形は未 commit だが断片2片が public 到達**した。
     **断片から完全形を推測するのは容易**（`<frag1>_<frag2><X>` という規則性）なので
     **保守的には変更を推奨**する。**これは私の過失によって新たに生じた判断事項である。**
