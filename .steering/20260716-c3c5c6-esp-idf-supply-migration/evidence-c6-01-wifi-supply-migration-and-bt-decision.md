...
| 旧 | 新 |
|---|---|
| `esp_wifi.cmake:318` `set(IDF_V554 $HOME/tools/esp-idf)` | **submodule 相対**（`${CMAKE_CURRENT_LIST_DIR}/../../../esp-idf`） |
| `esp_bt_idf61.cmake:50` `set(IDF $HOME/tools/esp-idf)`（"v5.5.4"を名乗る） | **`${IDF_V554}`＝submodule（真のv5.5.4タグ）** |
| `esp_bt_idf61.cmake:52` `set(IDF $HOME/tools/esp-idf-v6.1)` | **`ESP_IDF61_DIR` キャッシュ変数**（既定=従来パス／不在なら FATAL_ERROR で明示） |
...
>
> ⇒ **「BT だけ外部 v6.1 tree が必須」は撤回**（＝外部 `$HOME/tools/esp-idf-v6.1` への
...
> **実機（本DUT `<MAC-03>`・rev v0.2）：v554 は warm・真cold とも D-1 到達。**
