...
3. ★★**【撤回】** 初版で私はここに「**`PAIRING_COMPLETE` は発火していない**（`0x5DC0` が立っていない）」
   と書いた。**これは支持できない**（§11 でソースにより反証）。**`0x5DC0` の «不在» は
   この実装では何も証明しない**（ENC が常に PAIR を上書きするため原理的に観測不能）。
   加えて本セルは `app_status == BLE_HS_ENOTCONN` なので **NimBLE は PAIRING_COMPLETE を
   «意図的に送らない»**（`ble_sm.c:1111-1115`「Do not send this event on broken connection」）
   ⇒ **不在は二重に over-determined＝証拠価値ゼロ**。
   **bond 不成立の根拠は «マーカ» ではなく «ユーザーの観測（登録されていない）» と
   «ENC status=7＝暗号が確立していない» の2つ**であり、この2つで十分に支持される。
...
### 9.1 セル2 実測結果 ＝ **OK**（陽性対照 成立）

**ユーザー観測（逐語）**：「iPhone x C5 : **all ok（以前と同様に最初のdisconnect時にエラーは出る）**」

| reg | 生値 | タグで引いた意味 |
|---|---|---|
| `0x600B1000` | `0x5ade51c0` | SYNC 到達 |
| `0x600B1008` | `0xad000000` | adv rc=0 |
| `0x600B100C` | `0x00000000` | `ble_hs` reset 無し |
| **`0x600B1014`** | **`0x77170185`** | **WRITE(`0xABF3`) 発火**＝count=**1**・先頭バイト=**0x85** ⇒ **ユーザーは最後まで到達** |
| **`0x600B1018`** | **`0x5de00000`** | **ENC_CHANGE・status=`0x00`＝暗号確立成功** |
| `0x600B101C` | `0xa1020704` | bt_shim 割込みトレース（予約reg） |
| **`0x600B1020`** | **`0x604e0003`** | **CONNECT・status=0x00・count=`3`** |
| **`0x600B1024`** | **`0xd15c1303`** | **DISCONNECT・reason=0x13・count=`3`** |

**読み**：
- **暗号は確立した**（`ENC status=0`）＝**D-2d の実体（暗号必須 `0xABF4` が読める）をデバイス側からも支持**。
- **`0xABF3` WRITE 発火**（count=1・先頭 `0x85`）＝ユーザーは GATT 一式を通した。
- **接続3回・切断3回・全て reason `0x13`**＝**`BLE_ERR_REM_USER_CONN_TERM`＝リモート（iPhone）からの正常切断**。
  ⇒ ユーザーの「最初の disconnect でエラーは出るが**再コネクトできる**」と **count=3 が整合**。
  ★**これは «B の副作用» ではない**＝C5/C6 の従来挙動であり、**ユーザーが「以前は再コネクトできなかったので
  進んでいる・このままで良い」と判断済み**＝**回帰ではない・修正対象ではない**。
- ★`0x5DC0` が立っていないのは **異常ではない**（§11＝**この実装では原理的に立たない**）。
  `ENC status=0` の時 NimBLE は **PAIRING_COMPLETE を送ったうえで直後に ENC を送る**ので、
  **last-wins で必ず `0x5DE0` が最終値になる**。

### 9.2 事前登録した予測 P4 の記録

| 予測 | 事前登録値 | 結果 |
|---|---|---|
| **P4** | **B × iPhone が OK＝80%** | **★的中**（OK） |

### 9.3 ★帰属の解禁（事前登録した判定規則どおり）

**陽性対照 B × iPhone = OK** ⇒ **ビルド・ボード・BLE スタック・ハーネスは «今» 健全**
（同一ビルド `build/c5_tc_B_ble`・同一ボード・同一セッション帯）。
⇒ **セル1（B × Android）の NG は «装置の故障» では説明できない ＝ Android 固有**。
⇒ ★**帰属**：**「ESP-IDF v5.5.4 標準 toolchain（esp-14.2.0_20260121 / rv32imac）は
C5 の Android 問題に無関係」**＝**Android 調査の容疑者リストから toolchain を外してよい**。

★★**射程の限定（これ以上を書かない）**：
- 言えるのは **「toolchain を変えても Android の結果は変わらない」** まで。
  実測の根拠＝**病態が既知ベースラインと «数値まで» 同一**（`ENC=0x5de00007`）。
- **「Android の原因が何か」は依然として未特定**＝**本ラウンドの担当外**（RPA/SM 調査は別フェーズ・ユーザー判断）。
- **機序を語っていない**：`ENOTCONN` とユーザーの「ペアリング要求が来た」を接続していない。
  **RX/TX の計装をしていない**ので「要求に応答しなかった」のか「誤答した」のかは**区別できない**。

---

## 10. ★取らなかった対照と、その理由（no silent caps）

**取らなかった＝`A2 × Android`（ドリフト対照）。** 黙って省かず、理由を明示する：

- **理由(1)**：**A アームと B アームの Android 結果に «帰属すべき差» が無い**。
  B×Android は **歴史的ベースライン（3/3 NG・`ENC=0x5de00007`）と数値まで完全一致**しており、
  A も NG であることは既知。**両方 NG なら A/B の差分は存在せず、対照が結論を動かさない。**
- **理由(2)**：**環境ドリフト（「今 A も NG か」）の懸念は、陽性対照が独立に潰した**。
  `B × iPhone = OK` が **同一セッション帯・同一ボード・同一ビルド**で成立した＝
  **「装置が今壊れている」という対立仮説はこの1点で棄却済み**。
  ドリフト対照の目的（装置の健全性確認）は**より安く・より強く達成されている**。
- ★**この対照が結論を変えうるか＝変えうる唯一の筋**：もし **`A2 × Android` が «OK»** だったら、
  **「toolchain は Android に無関係」は偽**になる（A で通り B で落ちる＝toolchain が効いている）。
  **その可能性は低い**（既知 3/3 NG・かつ ISA 変更は no-op と実証済で A と B の BLE 差は GCC 版のみ）が、
  **ゼロとは言っていない**。**厳密には «A2×Android も NG» を実測していない**＝
  **本ラウンドの帰属は «既知ベースライン 3/3 が今も有効» という前提に1本依存している。**
  ⇒ **これを潰したければ `A2 × Android` を1セル追加すれば足りる**（コスト＝ユーザー1回）。

**その他 «測っていない» もの（正直に）**：
- **`B × B2`（ISA のみ差）の実機 run**＝未実施。ISA no-op は **機械語の一致**で示したが、
  **実機で反証する機会は作っていない**（§7.4 に既記）。
- **C3/C6 の実機**＝**一切触っていない**（ビルド非回帰のみ。ユーザー指示「C5 を先行」に従う）。
- **`-fsigned-char` が IDF ヘッダ境界で差を生むか**＝未検証（§3.3）。

---

## 10.1 セル3 ＝ **A2 × Android**（★§10 の穴を閉じる・ユーザー判断で実施）

**目的**＝**§10 で私が申告した «唯一の未測定の前提»**
（「Android の歴史的 3/3 NG が **今も** 成立する」）を**実測で閉じる**。
⇒ 閉じれば **帰属が «両アームとも今измった対照» の上に立ち、歴史的記録への依存が消える**。

### 10.1.1 ★どのバイナリで測るか＝**既存の A2 成果物を «再ビルドせずに» 使う**（判断と根拠）

**アーム**＝**A2 ＝ `esp-15.2.0_20251204`（GCC **15**）/ `rv32imc`**
（＝**C5 BLE の歴史的既知良好**＝**Android NG 3/3 を測った条件**。
★`A`＝`esp-14.2.0_20241119` は **W1 側**の既知良好であって本セルのアームではない）。
実測で同定：`build/c5_tc_A2_ble` の `CMAKE_C_COMPILER_AR` = `.../esp-15.2.0_20251204/...`／
`ESP32C5_IDF_STD_ISA=OFF`／ELF `Tag_RISCV_arch` = `rv32i2p1_m2p0_c2p0_...`（**A 拡張なし**）。

★**`-Wno-error=implicit-function-declaration` の扱い＝「セル1/2 と同一に揃える」を選ぶ。**
根拠（**mtime を実測**）：

| 成果物 | ビルド時刻 | ヘッダ状態 |
|---|---|---|
| 別エージェントの `hal_stub/include/esp_timer.h` 修正 | **15:09:49** | — |
| `build/c5_tc_B_ble/asp_flash.bin`（**セル1・2 が載せた実体**） | **14:21:10** | **修正前** |
| `build/c5_tc_A2_ble/asp_flash.bin`（**本セルで載せる実体**） | **14:21:08** | **修正前** |

⇒ **セル1/2 の B も、本セルの A2 も、どちらも «shadow 修正前・回避フラグ有り» の同一 vintage**。
∴ **既存 A2 成果物をそのまま焼けば、セル1/2 と «ヘッダ/フラグ条件が揃った» 比較になる**
（`md5 50afaecff6a7af2e5aa7764efce6aa9f`・**再ビルドしない**）。

★**再ビルドしてはならない**理由＝実測：`ninja -n` が **59 ステップ残** と答える
＝**今 ninja を回すと修正済みヘッダを取り込んで «別のバイナリ» になる**。
そうすると本セルは **«GCC15 vs GCC14» に «ヘッダ/コード生成の差» が重畳した2変数実験**になり、
**§10 の穴を閉じるどころか交絡を持ち込む**。
**歴史的ベースライン（Android NG 3/3）も shadow 修正が存在しない時代のもの**なので、
**修正前 vintage こそが «歴史的条件の再現» として妥当**。

### 10.1.2 ★★予測の事前登録（測定前に確定）

**P6：A2 × Android が NG である確率＝93%**（＝**OK である確率 7%**）
- 根拠：(i) このアームでの**歴史的 3/3 NG**、(ii) **B × Android が今 NG**で
  マーカが歴史値と**数値一致**（`0x5de00007`）、(iii) **A2 と B の差は GCC 版のみ**
  （ISA は §7 で **no-op と実証**）。
- ★**外れたら（＝A2 が OK だったら）何を意味するか**：
  **「toolchain は Android に無関係」という §9.3 の帰属は崩れる。**
  A2(GCC15)=OK・B(GCC14)=NG ⇒ **IDF 標準への整合（GCC14）が Android を «壊した» ＝重大な回帰**。
  ⇒ **その場合は §9.3 の帰属を明示的に撤回し**、
  **「BLE だけ esp-15.2.0 に留める」という «A だけ動く＝現行固定が正解» の型**（§3.4 P2 の外れ分岐）を
  結論候補として報告する。**この分岐を «起こらない» と決めてかかっていない。**

**★反証条件と、その «検算»（過去に反証条件自体が invalid だった実例に従う）**
- H（§9.3 の帰属）＝「toolchain を変えても Android の結果は変わらない」。
  **反証条件＝A2 × Android が OK**。
- ★★**反証条件の検算＝«偽の NG» と «偽の OK» は非対称である**：
  - **偽の NG は «作りやすい»**＝**bond ストアは RAM backed**なので、
    **真cold でデバイスは鍵を失う**。**スマホが鍵を持ったまま**だと不一致で落ちる
    ⇒ **これは «デバイスのバグ» ではなく «測定手順が作った交絡»**
    （本プロジェクトで **9時間を溶かした実際の事故**）。
    ⇒ **NG が出ても «forget したか» を必ず確認する。**
  - **偽の OK は «作りにくい»**（デバイスは鍵を失っているので、
    古い bond の再利用では成功しない）。
  ∴ **「A2=NG」は交絡でも起こりうるが、「A2=OK」はほぼ交絡では起こらない**
  ⇒ **反証（OK）が出たらそれは «強い» 信号**であり、**素直に帰属を撤回すべき**。
- ★★**フレッシュ性の判別子が «無い» ことを明記する**：
  コーディネータ／memory の「**`0x5DC0` が立てばフレッシュ**」は
  **§11 で私が «C5 では原理的に観測不能» と実証したので使えない**。
  ⇒ **本セルのフレッシュ性は «マーカ» では証明できない**。依拠するのは
  **(a) ユーザーへ渡す forget 手順、(b) ユーザーの観測（«ペアリング要求が来た» ＝
  フレッシュな pairing が試行された兆候）、(c) 真cold（デバイス側は確実に鍵ゼロ）** の3点のみ。
  ★**これはセル1（B × Android）にも同じく当てはまる既知の弱点**であり、
  **本ラウンドの Android 判定はこの弱点を共有している**（正直に記す）。

---

## 11. ★★計器の欠陥を発見：`0x5DC0`（PAIRING_COMPLETE）は **原理的に観測できない**

**ソースで確定**（`esp-idf/components/bt/host/nimble/nimble/nimble/host/src/ble_sm.c`）。
`ble_gap_pairing_complete_event` / `ble_gap_enc_event` の**全呼出しは3箇所のみ**：

```
1114:  ble_gap_pairing_complete_event(conn_handle, res->sm_err);   // if (enc_cb && app_status != BLE_HS_ENOTCONN)
1126:  ble_gap_enc_event(conn_handle, res->app_status, ...);       // if (enc_cb)
2902:  ble_gap_enc_event(proc->conn_handle, BLE_HS_ETIMEOUT, 0, 0);// timeout 経路（ENC のみ）
```

⇒ **PAIRING_COMPLETE は ENC_CHANGE の «前» に発火する**（1114 → 1126）。
⇒ **PAIR が発火して ENC が発火しない経路は存在しない**（両者とも `res->enc_cb` 依存）。
⇒ **C5 の «STORE6 を ENC/PAIR で共用・last-wins» 設計では、`0x5DC0` は必ず `0x5DE0` に上書きされる
＝ `0x5DC0` は最終値として決して観測されない。**

**影響（3点・すべて是正済み）**：
1. ★**アプリのコメントが事実と逆**：`ble_host_smoke_c5.c:126-136` は
   「**PAIRING は ENC の «後» に発火するので，成功時は `0x5DC0...`**」と書いているが、
   **ソースは逆**。⇒ **この判別子に乗ると «成功セルを失敗と誤読» する。**
2. ★**memory / 運用ルールの「判別子＝`PAIR` タグ `0x5DC0` が立てばフレッシュ」は
   «C5 のこの共用設計» には適用できない**（C3 は `0x54`/`0x58` の**別レジスタ**なので成立しうる＝
   **C3 の記録を C5 へ横滑りさせたのが誤りの源**と考えられる）。
3. ★**私自身がこの罠を踏み、§8.3 で撤回した**（「PAIRING 未到達」と書いた）。
   **セル2（iPhone・`ENC status=0`＝成功）でも `0x5DC0` は立っていない**という
   **決定的な反例**が出たことで発覚した＝**«健全な対照が計器を検定する»** の実例。
   **もしセル2を測らずセル1だけで報告していたら、「Android は PAIRING に到達しない」という
   誤った機序を記録に焼き付けていた。**

**恒久修正の提案（未実施・ユーザー判断）**：
- `ENC` と `PAIR` を**別レジスタに分ける**（C3 と同型）。C5 は STORE0-9 が全て使用中だが、
  **`STORE4`（割込み線1累積ミラー）は本ラウンド 3セルとも `0x00000000`＝情報価値が尽きている**
  ⇒ **`STORE4` を PAIR へ明け渡すのが最小コスト**（C3 が D-2c で採った「storm probe を
  無効化して reg を明け渡す」判断と同型）。
- あるいは **PAIR を «上書きされない» 形式にする**（例：ENC 側は `|= tag` で残す）。
- ★**それまでは「`0x5DC0` の不在」を証拠に使ってはならない。**

---

## 12. C5 最終状態（本ラウンドの締め）

### 12.1 ★本ラウンドの実質的な成果（当初の物語を訂正した上で）

★**当初の物語「C5 は汎用 GCC を使っていた」は前提が誤りだった**（実測：C5 の build dir 80 のうち
**汎用 GCC は 6 のみ**。164 件の汎用 GCC 事故は主に **C3/C6**）。

★★**C5 の真の欠陥は «W1 と W2 が同じコンパイラで検証されたことが一度も無かった» ことだった**：

| | 既知良好が使っていた実際のコンパイラ |
|---|---|
| **W1**（DHCP+ping） | `esp-14.2.0_20241119`（**IDF v5.5.0 の指定版**） |
| **W2**（BLE D-2c/D-2d） | `esp-15.2.0_20251204`（**IDF v6.1 の指定版・GCC 15**） |
| **真の v5.5.4 の指定版** `esp-14.2.0_20260121` | **0 構成＝一度も使われていない** |

＝**供給（ソース・blob）は真の v5.5.4 に統一済みなのに、コンパイラだけが
«ワークストリームごとにバラバラの、供給と一致しない版» だった。**

⇒ **本ラウンドで初めて解消**：**単一の `esp-14.2.0_20260121`（＝供給と同一ツリーが指定する版）で
W1 2/2・W2 BlueZ 2/2・W2 iPhone OK を «同時に» 達成**。**これが本ラウンドの見出し。**

### 12.2 C5 マトリクス（最終）

| 項目 | 結果 |
|---|---|
| **W1（DHCP+ping）真cold** | **PASS**（B 2/2：ping 40/0・40/0／A 3/3：4/0・44/0・45/0・IP `192.168.1.70`） |
| **W2 BlueZ hci0** | **PASS**（B 2/2・A2 2/2・`0xABF1`=`BT4-OK`） |
| **W2 iPhone** | **OK**（`ENC status=0`・`0xABF3` 発火・接続3/切断3 全て reason `0x13`＝正常） |
| **W2 Android** | **NG（不変）**＝`ENC=0x5de00007`（`ENOTCONN`）＝**既知ベースラインと数値一致** |
| **ISA `rv32imc`→`rv32imac`** | **証明された no-op**（atomic 命令 **0**・イメージ差 **2バイト＝`__TIME__` のみ**） |

### 12.3 ★「C5 の W1 真cold は初」か＝**確認した上で、限定して書く**

**コーディネータの申告を追検証した結果、«半分正しい»**：

- **電源断そのものは従来も本物だった**：`evidence-c5-01:136-140` は
  `usbhub3c_ctl.py off 5` ＋ **「読み戻しでデバイス消滅を確認（count 1→0, Vbus 0.02V）」**と記録＝
  **真の電源除去は行われていた**（「従来は電源断していない」と書くのは**誤り**）。
- **偽だったのは «観測したブートが POR ブートである» という部分**：
  同 evidence の採取は `rst:0x15 (USB_UART_HPSYS)`（`:145`）＝**コンソール open が誘発した
  EN リセットのブートを見ていた**。memory も
  「evidence-01/02/04 の「真の cold」は `rst:0x15`＝コンソール由来のリセット」と訂正済み。
  `evidence-c5-04:326` の **W1（真のcold）ping 30/0** も同じ採取系（`:239` に `rst:0x15`）。

⇒ **正確な主張**：**«POR ブートそのものを観測した W1» は本ラウンドが初**。
根拠＝(i) by-id 2ノード消失の読み戻し、(ii) **open が リセットしないことを対照実験で実証**
（アプリ実行中に open → バナー無し・ping 継続）、(iii) 全 W1 run で **ESP-ROM バナー数 0**
＝**open がリセットしていない＝走っているのは POR ブート**、(iv) sentinel（POR が LP_AON を消す）。
★**`rst:` 行そのものは «見えない»**（列挙が終わる前にバナーが出るため）。
**しかし `rst:0x1` は POR の証明にならない**（EN リセットと同一 signature）ので、
**上記の対照の方が強い証拠である**。

### 12.4 ★C6 の march を asp3_core で直す価値（ユーザー判断材料）

**ほぼ無い。** 理由＝**C5 で ISA 変更が «機械語として no-op» と実証された**
（同一コンパイラ・march のみ差で **`.text` は `__TIME__` の3バイト以外一致**・
最終イメージの A拡張命令数 **0**・WiFi/BLE 両アプリで独立確認）。
C6 も同じ blob 構成（**blob は A を要求も使用もしない**）である以上、
**`rv32imc`→`rv32imac` は C6 でもコード生成を変えない見込みが高い**。
⇒ **submodule（禁則）を触るコストに見合わない。**
★ただし **C6 で実測していない**ので「変わらない」と断定はしない（射程限定）。

### 12.5 申し送り（未解決・ユーザー判断）

1. ~~**`hal_stub/include/esp_timer.h` のヘッダ shadow**（§4）~~ ⇒ ★**本ラウンド終盤に解消（別エージェント）**。
   私が §4 で報告した「宣言2行で直る」を、**別エージェントが `hal_stub/include/esp_timer.h` に
   実装した**（`bool esp_timer_is_active` / `esp_err_t esp_timer_get_expiry_time` を
   本物からの逐語コピーで追加。**私は触っていない＝禁則を守った**。**現時点では作業ツリー上・未コミット**）。
   ★**実測で確認**：**`-DCMAKE_C_FLAGS=-Wno-error=...` を渡さずに C5 BLE が
   `configure rc=0 / build rc=0 / implicit-decl 0`**（`build/c5_noflag_probe`・IDF標準 GCC14）。
   ⇒ **§4 の «手渡しの回避に依存していた» 状態は解消された**。
   ⇒ **§7/§8/§9 の実機結果は «回避フラグ有り» で採ったもの**だが、
   **同じソースがフラグ無しでも通る**ことが示されたので、**回避は不要になった**
   （★ただし**フラグ無しビルドの実機 run は未実施**＝ビルド同一性までしか言っていない）。
   ★**なお別エージェントのコメントは「`bool` と暗黙 `int` の差はコード生成に出る（`snez` の有無）」と
   踏み込んでいる**。私の §4 の評価は「戻り値表現が ABI 上一致＝現状 benign」だった。
   **両者は «実害の有無» で対立しうる**が、**呼び先は `bool`（0/1 正規化済み）を返すので
   呼び側が正規化してもしなくても真理値は一致する**＝**私は依然 benign と読む**。
   **どちらにせよ宣言を足すのが正しい**ので**実務上の結論は同じ**であり、
   **この差は «未決» として残す**（実測で決着させていない）。
2. **DISC マーカの `&0xff` は lossy**（`0x200` を落とす）＝**既知欠陥**（§8.2）。
3. ★**`0x5DC0` 判別子は C5 では機能しない**（§11）＝**`STORE4` 明け渡しで直せる**。
4. **Android の機序は未特定・未計装**＝別フェーズ。
5. **`A2 × Android` 未実測**＝本ラウンドの帰属が依存する唯一の前提（§10）。

---

## 13. C3/C6 への転写手順（次フェーズの入力・**asp3_core 変更不要**）

**toolchain 固定と guard は既にチップ非依存**（`asp3/cmake/`）。転写は **target.cmake に2行**：

```cmake
#  asp3/target/esp32c6_espidf/target.cmake （C3 も同型）の先頭付近、
#  set(TARGETDIR ${CMAKE_CURRENT_LIST_DIR}) の直後に：
...
-DCMAKE_TOOLCHAIN_FILE=<repo>/asp3/cmake/toolchain-esp32-riscv32.cmake
   （-DRISCV64_TOOLCHAIN_PREFIX は不要になる）
```

**★転写前に踏む地雷（★本ラウンドで «私が実測した» ／伝聞ではない）**：
- **C3 の既定 WiFi ビルドは IDF 標準 GCC14 で落ちる**。実測（`build/c3_gcc14_probe`）：
  ```
  -DASP3_TARGET=esp32c3_espidf -DESP32C3_WIFI=ON, esp-14.2.0_20260121
    configure rc=0 / build rc=1 / error 数 = 1
    error: implicit declaration of function 'heap_caps_malloc'
  ```
  一方**同一構成を汎用 GCC 13.2.0 で建てると rc=0**（§5 の非回帰で実測）。
  ＝**C3 が今«通っている»のは、implicit-function-declaration が
  エラーでない GCC13 を使っているからにすぎない**（GCC14 以降は既定でエラー）。
  ⇒ **C3 に本 guard を入れると «今まで通っていたビルド» が即停止する**。
  ⇒ **C3 は «ヘッダ側（`heap_caps_malloc` の宣言）を直してから» 転写する**。
  **`-Wno-error` で黙らせるのは「潜在バグの隠蔽」＝§4 と同じ轍**。
  ★**壁は 1 件だけ**＝C3 転写のコストは «見えている» （C5 の `esp_timer` shadow と同族の
  ヘッダ衛生問題であり、構造的な壁ではない）。
- ★★**C6 は «無料で» 転写できる（実測）**：
  ```
  -DASP3_TARGET=esp32c6_espidf -DESP32C6_WIFI=ON, esp-14.2.0_20260121
    configure rc=0 / build rc=0 / error 数 = 0 / implicit-decl = 0
  ```
  ＝**C6 は IDF 標準 GCC14 でそのまま通る**。**ヘッダ修正も `-Wno-error` も不要**
  ⇒ **上記2行を `esp32c6_espidf/target.cmake` に足すだけで転写完了**（**asp3_core 不要**）。

**⇒ 転写コスト（実測サマリ）**

| チップ | IDF標準 GCC14 での既定 WiFi ビルド | 転写コスト |
|---|---|---|
| **C5** | rc=0（本ラウンドで実施済み） | **完了** |
| **C6** | **rc=0・error 0** | **2行だけ**（asp3_core 不要） |
| **C3** | **rc=1・error 1**（`heap_caps_malloc` の宣言欠落） | **2行 ＋ ヘッダ宣言1件の修正** |
- **C6 の `-march`** を IDF 標準へ揃えるには **asp3_core が要る**が、**§12.4 のとおり価値は薄い**。
  **toolchain 固定だけなら asp3_core 不要で転写できる**（＝**march と toolchain は分離して進められる**）。
