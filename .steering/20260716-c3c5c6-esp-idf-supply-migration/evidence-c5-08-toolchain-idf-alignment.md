...
DUT: **ESP32-C5 #1**（BASE MAC `<MAC-37>`, hub `1-6` port **3/4**）
...
> ## ★★【警告・C5 の BLE を触る者は先にここを読め】`0x5DC0` を判別子に使うな
>
> **「`PAIR` タグ `0x5DC0` が立てばフレッシュな bond」は C5 では成立しない＝原理的に観測できない。**
> ソースで確定（§11）：`ble_sm.c` は `pairing_complete`(:1114) を `enc_event`(:1126) の **前** に発火し、
> 両者とも `res->enc_cb` 依存＝**PAIR だけ発火する経路が無い**。C5 は両者を **STORE6 に last-wins で共用**
> するため **ENC が必ず PAIR を上書きする**。
> - **`ble_host_smoke_c5.c:126` のコメントは «PAIRING は ENC の後に発火» と書いているが逆＝誤り。**
> - **MEMORY.md にも «判別子＝`0x5DC0`» と誤って載っていた**（本ラウンドで親へ報告・訂正済み）。
> - **実害**：この判別子に乗ると **«成功したセルを失敗と誤読» する**。私は実際に踏み §8.3 で撤回した
>   （**発覚したのは陽性対照セル2＝`ENC status=0` の成功セルでも `0x5DC0` が立たなかったから**）。
> - **修理**：**`STORE4`(`0x600B1010`) に PAIR 専用レジスタを与えよ**
>   （**3セルとも `0x00000000`＝情報価値が尽きており空き**。C3 が D-2c で採った「reg を明け渡す」判断と同型）。
> - **それまでは「`0x5DC0` の不在」を証拠として使ってはならない。**
...
| `$HOME/tools/esp-idf-v6.1` | `esp-15.2.0_20251204` |
...
### 7.5 スマホセル1 の準備手順（当時の記録。**結果は §8**／全3セル完了済み＝§8・§9・§10.1）
...
- **広告在席は «スキャンで» 確認**（`RSSI=-41`・`<MAC-37>`）。**esptool は使っていない**
...
> ★★**【後日解決】本節は «A2 × Android を取らなかった» ことの説明として書かれたが、
> ユーザー判断により **実施した**（§10.1＝セル3・**結果 NG**）。**穴は閉じた。**
> 以下は当時の判断の記録として保全する（**当時の私の「1本の未測定の前提に依存している」という
> 申告は正しく、実際にその1本が本ラウンド内で潰された**）。

**（当時）取らなかった＝`A2 × Android`（ドリフト対照）。** 黙って省かず、理由を明示する：
...
### 10.1.3 セル3 実測結果 ＝ **NG**（＝穴が閉じた）

**ユーザー観測（逐語）**：「Android x C5 : **同様にペアリング要求後に切れる．登録されない**」

★**セル1（B=GCC14）との差分＝全9レジスタで «差分ゼロ»**（目視でなくスクリプトで比較）：

| reg | 名前 | **セル1**（B・GCC14・IDF標準） | **セル3**（A2・GCC15・歴史的） | 一致 |
|---|---|---|---|---|
| `0x600B1000` | SYNC | `0x5ade51c0` | `0x5ade51c0` | YES |
| `0x600B1008` | ADV rc | `0xad000000` | `0xad000000` | YES |
| `0x600B100C` | ble_hs reset | `0x00000000` | `0x00000000` | YES |
| `0x600B1010` | intr line1 | `0x00000000` | `0x00000000` | YES |
| `0x600B1014` | **WRITE `0xABF3`** | `0x00000000` | `0x00000000` | YES |
| `0x600B1018` | **ENC/PAIR** | **`0x5de00007`** | **`0x5de00007`** | **YES** |
| `0x600B101C` | bt_shim trace | `0xa1020704` | `0xa1020704` | YES |
| `0x600B1020` | **CONNECT** | `0x604e0001` | `0x604e0001` | YES |
| `0x600B1024` | **DISCONNECT** | `0xd15c1301` | `0xd15c1301` | YES |

**読み**（タグで引く。`0x5DC0` は使わない＝§11 で観測不能と実証済）：
- **`ENC = 0x5de00007` ＝ `BLE_HS_ENOTCONN`(7)** ＝ **セル1 と一致・歴史的ベースライン(3/3) とも一致**。
- **接続1回・切断1回（reason `0x13`）・`0xABF3` 未発火** ＝ セル1 と一致。
- ⇒ **A2（GCC15・歴史的既知良好）でも Android は NG。**

**P6 の記録**：

| 予測 | 事前登録値（commit `2e87827`） | 結果 |
|---|---|---|
| **P6** | **A2 × Android が NG＝93%**（OK＝7%） | **★的中**（NG） |

### 10.1.4 ★forget 手順の検定（＝自分で登録した非対称を、自分に適用する）

私は §10.1.2 で「**偽の NG は作れるが偽の OK は作りにくい**」と登録した。
**NG が出た以上、forget 手順を疑う義務がある。** 疑ったうえで書く：

**(a) デバイス側のフレッシュ性は «証明済み»**：真cold（by-id 2ノード消失の読み戻し）＋
**POR が LP_AON を消すことを sentinel で実証**（§7.1）⇒ **bond ストアは RAM backed なので
デバイスの鍵は確実にゼロ**。**ここに疑いの余地は無い。**

**(b) スマホ側のフレッシュ性は «証明できない»**：依拠できるのは
ユーザーの forget 操作と、「**ペアリング要求が出た**」という観測のみ。
★**«ペアリング要求が出た» はフレッシュの証明にならない**（＝コーディネータの問いへの回答は **«ならない»**）：
鍵を保持したスマホでも、**古い LTK での暗号化に失敗した後に «再ペアリングへフォールバックする»
実装は普通に存在する**ので、**プロンプトの存在は «stale 由来の再ペアリング» を排除しない**。
**«弱い整合» ではあるが «証拠» とは呼べない。**（過大評価しない。）

**(c) ★それでも本ラウンドの帰属は «汚染に対して頑健» である＝理由**：
本ラウンドの主張は **«差分の主張»**（toolchain を変えても結果が変わらない）であって
**«絶対値の主張»**（C5 の Android は壊れている）ではない。
**もし仮に Android 両セルが «スマホ側 stale bond» で汚染されていたとしても、
その汚染は «toolchain 非依存» の共通要因＝両アームに等しく効く common-mode**であり、
**A/B の «差» では相殺される**。**セル1 と セル3 が «全9レジスタ差分ゼロ» であることは、
汚染仮説の下でも «toolchain が結果を変えない» ことと矛盾しない。**
⇒ **差分の主張（§10.1.5）は汚染に頑健。汚染が壊すのは «C5×Android は実バグである» という
絶対値の主張の方であり、本ラウンドはそれを主張していない**（§10.1.5 の「書いていないこと」参照）。

**(d) 補助的事実**：**同一セッション・同一ボード・同一 forget 運用で セル2（iPhone）は OK になった**
⇒ **運用が «系統的に» 壊れているわけではない**ことの弱い支持
（★ただし forget したのは別の端末なので、**Android 側の forget が効いた証明にはならない**）。

### 10.1.5 ★★帰属の最終形（3セルすべて «今»・同一セッション・同一ボード）

| セル | アーム | 相手 | 結果 |
|---|---|---|---|
| **1** | **B**＝esp-14.2.0_20260121（**GCC14**）/ **rv32imac**＝**IDF標準** | Android | **NG**（`ENC=0x5de00007`） |
| **2** | **B**（同上） | iPhone | **OK**（`ENC status=0`・`0xABF3` 発火）＝**陽性対照** |
| **3** | **A2**＝esp-15.2.0（**GCC15**）/ rv32imc＝**歴史的既知良好** | Android | **NG**（`ENC=0x5de00007`・**セル1と全9レジスタ一致**） |

⇒ ★**言えること（実測の上に立つ）**：
1. **両アームとも «今» 測って Android は NG** ⇒ **§10 で私が申告した «唯一の未測定の前提»
   （歴史的 3/3 NG が今も成立するか）は閉じた。歴史的記録への依存が消えた。**
2. **陽性対照（セル2）が装置の健全性を独立に実証** ⇒ **NG は «ビルド／ボード／BLE スタックの故障» ではない。**
3. ⇒ **「ESP-IDF v5.5.4 標準 toolchain（GCC14 / rv32imac）は C5 の Android 問題に無関係」**が
   **両アームの実測対照の上に立つ**。
   **B は Android を «壊してもいないし直してもいない»**（`0x5de00007` は3者で完全一致）。
   ⇒ **Android 調査の容疑者リストから toolchain を外してよい。**

⇒ ★**書いていないこと（意識的に）**：
- **«なぜ Android が落ちるか»** ＝ **未特定・未計装・本ラウンドの担当外**。
- ★**«セル1 と セル3 は同じ機序» とは言っていない**。**一致しているのは
  «ENC status とユーザー観測とマーカ9本の値» まで**であり、**病態の一致 ≠ 機序の同一**。
  （マーカは «どこで落ちたか» を語る解像度を持たない＝§11 の欠陥もある。）
- ★**«C5 の Android は直らない» のような一般化はしない**。**測ったのは toolchain 軸だけ**であり、
  RPA/SM/供給/ホスト設定など他の軸は**本ラウンドで一切動かしていない**。

---

## 11. ★★【次に C5 の BLE を触る者へ・必読】計器の欠陥：`0x5DC0`（PAIRING_COMPLETE）は C5 で **原理的に観測できない**
...
| **W2 iPhone** | **OK**（B・`ENC status=0`・`0xABF3` 発火・接続3/切断3 全て reason `0x13`＝正常） |
| **W2 Android** | **NG — ただし toolchain 不問**：**B(GCC14)・A2(GCC15) の2アームを «今» 実測し
**全9マーカーが差分ゼロ**（`ENC=0x5de00007`）＝**歴史的 3/3 とも一致** |
| **ISA `rv32imc`→`rv32imac`** | **証明された no-op**（atomic 命令 **0**・イメージ差 **2バイト＝`__TIME__` のみ**） |
| **`esp_timer.h` shadow** | **解消（別エージェント）**⇒**回避フラグ `-Wno-error=implicit-function-declaration` は不要**
（実測：`build/c5_noflag_probe` が rc=0・implicit-decl 0） |
...
### 12.4.1 ★事前登録した予測の総括（全件・的中/外れ）

| # | 予測（事前登録） | 値 | 結果 |
|---|---|---|---|
| **P1** | B × W1 が通る | 92% | **★的中**（B 2/2 PASS） |
| **P2** | B × W2(BlueZ) が通る | 85% | **★的中**（B 2/2 PASS） |
| **P3** | march の A拡張／blob 整合 | — | **★予測ではない**（先に実測した旨を §3.4 に明記）＝A は入るが **no-op**・blob と整合 |
| **P4** | B × iPhone が OK | 80% | **★的中**（OK） |
| **P5** | B × Android が **OK に変わる** | 12% | **★的中**（88% 側＝変わらなかった） |
| **P6** | A2 × Android が **NG** | 93% | **★的中**（NG・セル1と全9レジスタ一致） |
| **H** | ISA 変更は挙動に影響しない | — | **反証されず**（機械語一致）。★**実機で B vs B2 を走らせていない＝反証の機会を作っていない**（正直に） |
| **H2** | B は既知良好を壊さない | — | **反証されず**（W1 2/2・BlueZ 2/2・iPhone OK） |
| **C0相当** | （FATAL の逃げ道が効くこと） | — | **★外れ→自己修正**：`-DASP3_ESP_EXPECTED_TOOLCHAIN=` が**効かなかった**＝
**自分のエラーメッセージが嘘をついていた**。実際に発火させたから捕まえた（§2.2） |

★**予測が «全部当たった» ことを手柄にしない**：P1 は **A≡B がバイト同一**と分かった時点で
ほぼ自明であり（§3.2）、**当てやすい予測を並べたに過ぎない**。
**本ラウンドで «外れた» のは C0（私の FATAL の逃げ道）だけ**で、**そこが唯一の実質的な学び**だった。
...
2. **DISC マーカの `&0xff` は lossy**（`0x200` を落とす）＝**既知欠陥**（§8.2）。
   ⇒ **reason を `0x200` 込みで保存する形式へ変えれば直る**（16bit 使えば足りる）。
3. ★★**`0x5DC0` 判別子は C5 では «原理的に» 機能しない**（§11・冒頭警告）＝
   **`STORE4`(`0x600B1010`) に PAIR 専用レジスタを与えれば直る**（3セルとも 0＝空き）。
   **併せて `ble_host_smoke_c5.c:126` の «PAIRING は ENC の後» というコメントを訂正すること**（逆＝誤り）。
4. ★**Android の機序は未特定・未計装**＝**別フェーズ**（ユーザー判断）。
   **次に必要なのは «どこで落ちるか» を見る計装**＝**SMP の RX/TX を数える**
   （C3 の evidence-c3-04/05 が採った型：`--wrap` で HCI EVT / ACL / SMP PDU を計数し
   «最後に流れた SMP PDU» を特定する）。
   ★**その際の必須の前提**（本リポジトリの実績）：
   - **計装は無害と仮定するな**＝**まず «健全な対照»（＝本ラウンドなら B × iPhone）に計装を当てて
     C0（非破壊性）を確認**してから被験体に当てる（hot path の dump が bond を壊した実例がある）。
   - **`--wrap` は «実際に噛んだか» を逆アセンブルで確認**（噛まない wrap の沈黙を
     «被験体の沈黙» と読む捏造の寸前まで行った実例がある）。
   - **フレッシュ性の判別子が無い**（§10.1.4(b)）＝**forget 手順に依存する**。
     **判別子が要るなら先に §11 の修理（`STORE4`）をやること。**
5. ~~**`A2 × Android` 未実測**~~ ⇒ ★**実施済み（§10.1・セル3）＝NG＝穴は閉じた。**
6. ★**Android 2セルの «スマホ側» フレッシュ性は証明されていない**（§10.1.4(b)）。
   **差分の主張には影響しない**（common-mode なので相殺・§10.1.4(c)）が、
   **«C5×Android は実バグである» という絶対値の主張をしたいなら、ここを潰す必要がある**。
