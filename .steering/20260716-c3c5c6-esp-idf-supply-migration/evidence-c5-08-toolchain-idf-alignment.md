...
1. ★**`hal_stub/include/esp_timer.h` のヘッダ shadow**（§4）＝**宣言2行で直る**。
   現状 C5 BLE は **`-DCMAKE_C_FLAGS=-Wno-error=implicit-function-declaration` 手渡し無しでは
   ビルド不能**。**`hal_stub/` は3チップ共有かつ別エージェントが編集中**のため**未実施**。
   隠れている警告は **2件のみ**（実体は ELF に在り・戻り値型 ABI 整合＝**現状 benign**）。
