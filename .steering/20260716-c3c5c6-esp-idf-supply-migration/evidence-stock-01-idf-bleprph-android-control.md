...
DUT: **C6 `<MAC-03>`（`1-6.2.3`／電源 `-p 2`）** ・ **C5#1 `<MAC-37>`（`1-6.4`／電源 `-p 3-4`）**
...
```
esp32c6: $HOME/.espressif/tools/riscv32-esp-elf/esp-14.2.0_20260121/.../riscv32-esp-elf-gcc
esp32c5: 同上
...
|---|---|---|---|---|
| C6 `<MAC-03>` | ESP32-C6FH4 rev **v0.2** | **4MB** | `tmp/asp3_flash_backup/c6_asp3_full_4MB.bin` | `2f74e55a7f4c3eb8d729a070797412cf` |
| C5 `<MAC-37>` | ESP32-C5 rev **v1.0** | **8MB** | `tmp/asp3_flash_backup/c5_asp3_full_8MB.bin` | `c29621d518f49d7f27d6f1b0221f8dee` |
...
cd $HOME/TOPPERS/ASP3CORE/asp3_esp_idf
# C6 を ASP3 へ戻す
...
| 項目 | 実測 |
|---|---|
| DUT | **C6 `<MAC-03>`**（ESP32-C6FH4 rev v0.2・`1-6.2.3`） |
| **広告名** | **`IDFCTL-C6`** |
| **BLE アドレス** | **`<MAC-05>`**（＝base MAC `…5A:9C` **+2**。★**名前でなく MAC が個体の証拠**） |
...
```
<MAC-05>  name='IDFCTL-C6'      <<< C0 DUT（stock bleprph・真cold）
<MAC-37>  name='ASP3-C5-BLE'    <<< C5（まだ ASP3。未 flash）
<MAC-32>  name='FMP-ESP32-BLE'    <<< 別プロジェクト（触るな）
<MAC-41>  name='FMP-ESP32S3-BLE'  <<< 別プロジェクト（触るな）
```
...
1. ★**Android・iPhone の «両方» で、以下を forget（ペアリング解除）してから始める**：
   - **`IDFCTL-C6`**
   - ★**`ASP3-C6-BLE`**（**同じ MAC `<MAC-05>` の «前の名前»**＝
...
2. **Android で BLE スキャン**（nRF Connect 等）→ ★**`IDFCTL-C6`（`<MAC-05>`）を選ぶ**。
...
| MAC | 広告名 | 正体 |
|---|---|---|
| `<MAC-41>` | **`FMP-ESP32S3-BLE`** | **別プロジェクト（hub `1-5` port 3）** |
| `<MAC-32>` | **`FMP-ESP32-BLE`** | **別プロジェクト（ESP32・USB 接続でない可能性）** |
| `<MAC-37>` | `ASP3-C5-BLE` | 我々の C5（ASP3） |
...
⇒ ★**判定は名前だけに頼らず MAC を併用**（`<MAC-05>` / C5 は `<MAC-37>`）。
...
|---|---|---|---|---|
| 0.0 | connection established | `<MAC-15>` | enc=0 **bond=0** | status=0 |
...
| 100.8 | connection established | `<MAC-31>` | enc=0 bond=0 | status=0 |
| 106.7 | **encryption change** | 同 | **enc=1 bond=1** | **status=0** |
| 110.4 | **encryption change** | `<MAC-16>` | enc=0 bond=0 | ★**status=7** |
| 111.4 | connection established | `<MAC-13>` | enc=0 bond=0 | status=0 |
...
| 項目 | 実測 |
|---|---|
| DUT | **C5#1 `<MAC-37>`**（ESP32-C5 rev v1.0・`1-6.4`／電源 `-p 3-4`） |
| **広告名** | **`IDFCTL-C5`**（★**名前でチップを判別できる**＝C6 と取り違えない） |
| **BLE アドレス** | ★**`<MAC-38>`**（**実測**。base MAC `…A7:44` **+2**＝C6 と同じ規則） |
...
```
**真cold ブートログを capture が捕捉**（`Device Address: <MAC-38>`・
`GAP procedure initiated: advertise`）＝**stock は C5 でも «真cold から» 起動して広告する**。
**BlueZ fresh discovery で在席実証**：`<MAC-38> name='IDFCTL-C5' Paired=False`。
...
1. ★**Android・iPhone の «両方» で forget**：**`IDFCTL-C5`** ／ ★**`ASP3-C5-BLE`**
   ...
   **C0 で bond した `IDFCTL-C6` も消しておくと安全**（**stock は RAM-backed＝
   ...
   → **forget 後に BT OFF→ON**。
2. **BLE スキャン → `IDFCTL-C5`（`<MAC-38>`）を選ぶ**。
...
| 最長セッション | `<MAC-12>`：**550s..835s（285秒）**・**`bonded=1` ×25**・**暗号必須特性の READ ×1** |
| 同時期の C6 | `<MAC-25>`：**1614s..1716s（102秒）**・`bonded=1` ×2（**C0 の «後» の新規セッション**） |
...
| 我々の C5 がリンクする blob | **同一パス**（`$HOME/TOPPERS/asp3_esp_idf` は **同一 repo への symlink**） |
...
| 在席 | **`<MAC-38>` `IDFCTL-C5` `Paired=False`**（スキャンで実証。**esptool は使っていない**） |
...
3. **BLE スキャン → `IDFCTL-C5`（`<MAC-38>`）へ接続**。
...
| 項目 | 実測 |
|---|---|
| 広告名 / BLE addr | **`IDFCTL-C5SC`** / `<MAC-38>`（★**arm-1 と «同じアドレス»**＝**forget 必須**） |
...
2. **Android で forget**：**`IDFCTL-C5SC`** ／ **`IDFCTL-C5`**（★**同じアドレス `<MAC-38>`＝
   前アームの bond が残っていると汚染します**）／ **`ASP3-C5-BLE`** → **BT を OFF→ON**。
3. **スキャン → `IDFCTL-C5SC`（`<MAC-38>`）へ接続**。
...
|---|---|---|---|---|
| **#1** @0.0s | `<MAC-17>`(t1=RPA) | `<MAC-17>`(t1) | **未解決** | **12.6s＝フルペアリング** |
| （#1 のペアリング後） | `<MAC-17>`(t1) | ★**`<MAC-34>`(t0=public)** | ★**解決** | — |
| **#2** @39.7s | ★**`<MAC-30>`(t1)＝別の RPA** | ★**`<MAC-34>`(t0)＝同一 identity** | ★**接続時点で解決** | ★**0.1s＝LTK 再利用** |
...
**同じチップ（ESP32-C5 rev v1.0・同一個体 `<MAC-37>`）・
...
| **BLE アドレス（実測）** | ★**`<MAC-20>`**＝**base MAC `…BA:BC` +2**（**C5/C6 と同じ規則**）。★**ASP3 は `…BA:BC`（base そのもの）で広告していた**＝**我々と stock で BLE アドレスが違う**（C5 と同じ差） |
| 真cold | `-p 1` off → **by-id 消滅を読み戻し**（`c3=GONE`）→ on。**C5/C6 は GONE のまま** |
| ログ | **新規ファイル**・**開始時 `connection established`=0**・**リセットバナー 0** |
| **在席** | ★**`<MAC-20>` `IDFCTL-C3SC` `Paired=False`**／**我々のボードで電波に出ているのは «1枚だけ»（実測）** |
...
|---|---|---|---|---|
| **#1** @0.0s | `<MAC-27>`(t1=RPA) | `<MAC-27>`(t1) | **未解決** | **6.5s＝フルペアリング** |
| （#1 ペアリング後） | 同 | ★**`<MAC-34>`(t0=public)** | ★**解決** | — |
| **#2** @21.7s | ★**`<MAC-14>`(t1)＝別 RPA** | ★**`<MAC-34>`** | ★**接続時点で解決** | ★**0.2s＝LTK 再利用** |
| **#3** @36.8s | `<MAC-14>`(t1) | ★**`<MAC-34>`** | ★**解決** | ★**0.2s＝LTK 再利用** |

- **解決イベント＝13/14**（**唯一の «未解決» は conn#1＝構造上の正解**＝IRK はペアリング «中» にしか配られない）。
- ⇒ ★★**アームは生きている**（**IRK が実際に交換され、controller が RPA を解決した**）。
- ★**副産物＝`peer_id` が `<MAC-34>`＝§15 の C5 D-7 セルと «同一の identity»**
...
| BLE アドレス | `<MAC-20>` / 広告名 `IDFCTL-C3SC` / `Paired=False` |
...
3. **iPhone でスキャン → `IDFCTL-C3SC`（`<MAC-20>`）へ接続**。
4. ★**判定してほしいこと＝«4 つ»**：
...
| 容疑者 | 外した根拠（実測） |
|---|---|
| **チップ（ESP32-C5 / C3 のシリコン）** | **同一個体**（`<MAC-37>` / `<MAC-19>`）**で stock は 3 軸とも通る** |
...
| ★**機能検証** | ★★**真cold → BlueZ スキャンで `ASP3-C3-BLE @ <MAC-19>` を確認**
...
|---|---|---|---|---|
| **C5** `<MAC-37>` | `Hash of data verified` | ★`verify OK (digest matched)` | ★**dump == `build/c5_tc_A2_ble/asp_flash.bin`（True）** | ★**`ASP3-C5-BLE @ <MAC-37>` を実測** |
| **C6** `<MAC-03>` | `Hash of data verified` | ★`verify OK (digest matched)` | ★**dump == `build/gd_c6_ble/asp_flash.bin`（True）** | ★**`ASP3-C6-BLE @ <MAC-03>` を実測** |
| **C3** `<MAC-19>` | （§21.1） | `verify OK` | **dump == `build/rb_A_ble/asp_flash.bin`（True）** | **`ASP3-C3-BLE @ <MAC-19>` を実測** |
...
★**実測で同定した identity**：
- **`<MAC-34>`（type 0＝public）＝ユーザーの Android**
  ——★**C5 D-7 セルと C3 D-7 セルで «同一の identity»**＝**同じ端末が両セルを実施したことが実測で確定**。
- **`<MAC-35>`（type 0＝public）＝ユーザーの iPhone**（C3 D-7 セル）。
