...
| D-1 | Bluetooth（BLE）コントローラ起動＋VHCIループバック | **実施中**（ビルド成功・実機でコントローラ起動〜enable開始まで到達。`libbtdm_app.a`/ROM内部アサート（`emi.c:164`＝EMページ所有権テーブル不整合）でブロック。JTAG調査によりheap/config値/ble_max_actは原因ではないと確認済み、真因はクローズドソース内部実装のため未特定。`docs/bt-shim.md`） |
| ESP32-C6横展開（B-0/B-1） | asp3_core Phase A完了（`feat/esp32c6`）を受けたesp-hal統合。`asp3/target/esp32c6_espidf/`を新設（C3版と並列） | **完了**（実機でsample1起動・**test_porting 6/6 PASS**。QEMU非対応＝実機専用。`docs/hal-integration.md`） |
| ESP32-C6横展開（B-2a Wi-Fi scan） | C3のWi-Fi os_adapter shimをC6へ移植（割込みコントローラ分離＝INTMTX+PLIC_MX・RNG/eFuseレジスタ差替え・modem_clock実ソース採用・リンカスクリプトIRAM/DRAM区画追加） | **実施中**（`phy_init`のBreakpoint例外は根本原因特定・修正済み。実機で`esp_wifi_init/start/scan_start`成功・スキャン完走するが発見AP数0。アンテナ接続下での対照実験でハードウェア要因は排除。MAC制御ブロック16KB・modem syscon 4KBは既知良品とバイト単位で完全一致。WIFIPWRクロックドメイン未有効化・WiFi MAC割込み線のLEVEL型未設定の2件を実機レジスタ比較で発見・修正済み（正当な修正として維持）だが，どちらも単独では解消せず。**同一基板でNuttXのWi-Fiドライバをビルド・実行し`wapi scan`が実AP 6件を検出**（os_adapter shim方式そのものがこのシリコンで動作する動かぬ証拠）。継続ポーリング計装でNuttX側はMAC割込みソース（`INTMTX_STATUS0`bit0）が実際にアサートされることを確認した一方，ASP3側は同等の高密度サンプリング（4702回／約2.4秒）でも一度もアサートされないことを確認＝ハードウェア／blob側の問題ではなくASP3自作shim側に未発見の具体的な不具合が残っていることが確定。osi関数の全フィールドはNuttXと構造的に一致（欠落なし）。RXバッファのDMA対応メモリ要求・キャッシュコヒーレンシ仮説も検証したが，NuttXが同フィールドで無条件の汎用ヒープを使い動作している実績，およびC6のキャッシュ／MMUは外部メモリ（Flash/PSRAM）専用でSRAM/DRAMは非対象という事実の両方から棄却。主要な仮説クラス（静的レジスタ・osi構造・ハードウェア欠陥・DMA/キャッシュ）をすべて実測で潰した状態で一旦区切り，次段階（blobの単一命令ステップ実行によるNuttX/ASP3比較）は別リソースへ引き継ぎを推奨。`docs/wifi-shim-c6.md`） |
| ESP32-C5横展開（B-0〜B-2a） | C6テンプレートからのESP32-C5移植（詳細設計＝`docs/c5-port-design.md`。チップ依存部は本リポジトリ側`asp3/arch/riscv_gcc/esp32c5/`に新設＝submodule外配置・将来asp3_coreへupstream予定。C5は標準RISC-V CLIC採用のため割込み基盤（トラップベクタ・mintthresh CSR・INTNO↔CLIC線変換）を再設計。Wi-Fi shimはRNG/eFuse/MODEM_SYSCONレジスタ差替え・CLIC対応の割込みwrapper・esp32c5.rom.eco3.ldによるphy_get_max_pwr実体化） | **実施中**（ビルドのみ完了・実機未検証：sample1／test_porting／wifi_scan（`-DESP32C5_WIFI=ON`，FLASH 10.92%・RAM 75.96%）の3構成がエラー0でリンク成功。C6のdeaf-RX切り分けを兼ねる＝C5でscanが成功すればC6固有問題と確定。実機投入時の確認事項は`docs/c5-port-design.md`§8参照） |
