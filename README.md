...
| ESP32-C6横展開（B-2a Wi-Fi scan） | C3のWi-Fi os_adapter shimをC6へ移植（割込みコントローラ分離＝INTMTX+PLIC_MX・RNG/eFuseレジスタ差替え・modem_clock実ソース採用・リンカスクリプトIRAM/DRAM区画追加） | **実施中**（ビルド・リンク0エラー・実機で`esp_wifi_init()`成功まで到達。`phy_init`内でBreakpoint例外により停止＝libphy.a内部assert相当と推定・原因未特定。`docs/wifi-shim-c6.md`） |
