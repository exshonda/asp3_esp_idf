...
| ESP32-C6横展開（B-2a Wi-Fi scan） | C3のWi-Fi os_adapter shimをC6へ移植（割込みコントローラ分離＝INTMTX+PLIC_MX・RNG/eFuseレジスタ差替え・modem_clock実ソース採用・リンカスクリプトIRAM/DRAM区画追加） | **実施中**（`phy_init`のBreakpoint例外は根本原因特定・修正済み＝`periph_module_reset()`のenum範囲外アクセスをGCCがtrapへ最適化していた。実機で`esp_wifi_init/start/scan_start`成功・スキャン完走するが発見AP数0＝JTAGでWiFi MAC割込みソース自体が一切アサートされないことを確認済み。アンテナ接続下での対照実験（素のESP-IDFは同一基板でAP 10個超検出）でハードウェア要因は排除。MAC制御ブロック16KB・modem syscon 4KBは既知良品とバイト単位で完全一致（静的レジスタ誤設定は考えにくい）。blob内蔵ログゲート`g_log_level`の解読により，blobの正常スキャン完了診断（`scan_done: arg=...`）が一度も実行されていないことが判明＝観測している`SCAN_DONE`はタイムアウト/フォールバック経由の疑い。原因はMAC内部の未文書化なRXアーム手順または較正結果反映側に絞り込み中，未特定。`docs/wifi-shim-c6.md`） |
