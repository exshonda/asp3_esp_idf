...
| ESP32-C6横展開（B-2a Wi-Fi scan） | C3のWi-Fi os_adapter shimをC6へ移植（割込みコントローラ分離＝INTMTX+PLIC_MX・RNG/eFuseレジスタ差替え・modem_clock実ソース採用・リンカスクリプトIRAM/DRAM区画追加） | **実施中**（`phy_init`のBreakpoint例外は根本原因特定・修正済み＝`periph_module_reset()`のenum範囲外アクセスをGCCがtrapへ最適化していた。実機で`esp_wifi_init/start/scan_start`成功・スキャン完走するが発見AP数0＝WiFi MAC割込みソース自体が一切アサートされないことを確認済み。アンテナ接続下での対照実験（素のESP-IDFは同一基板でAP 10個超検出）でハードウェア要因は排除。MAC制御ブロック16KB・modem syscon 4KBは既知良品とバイト単位で完全一致。WIFIPWRクロックドメイン（`modem_lpcon.clk_wifipwr_en`）未有効化を実機レジスタ比較で発見・修正済み（正当な修正として維持）だが，これだけでは解消せず＝「クロック/電源ドメイン有効化漏れ」という仮説クラス全体を実測で棄却。原因はMAC内部（blob非公開領域）のRX-arm/開始トリガのシーケンス自体に絞り込み中，静的レジスタ比較では検出不能なクラスの不具合＝実機JTAG単一命令ステップ実行が必要。複数の有望な手がかりが精査の結果「赤信号」と判明する状況が続いたため，このセッションでの継続よりも別リソースへの引き継ぎ・方針再検討を推奨する段階。`docs/wifi-shim-c6.md`） |
