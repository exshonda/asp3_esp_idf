...
| ESP32-C6横展開（B-2a Wi-Fi scan） | C3のWi-Fi os_adapter shimをC6へ移植（割込みコントローラ分離＝INTMTX+PLIC_MX・RNG/eFuseレジスタ差替え・modem_clock実ソース採用・リンカスクリプトIRAM/DRAM区画追加） | **実施中**（`phy_init`のBreakpoint例外はJTAG調査で根本原因特定・修正済み＝`periph_module_reset()`のenum範囲外アクセスをGCCがtrapへ最適化していた。`modem_clock_module_mac_reset()`へ修正後，実機で`esp_wifi_init/start/scan_start`成功・スキャン完走するが発見AP数0＝JTAGでWiFi MAC割込みソース自体が一切アサートされないことを確認済み・原因はMAC/PHY側に絞り込み未特定。`docs/wifi-shim-c6.md`） |
