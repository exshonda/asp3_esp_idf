/*
 *  C5負荷試験デモ（TCP echo + UDP echo + ヒープ/カウンタ監視）
 *
 *  apps/load_test_c3（実施1，docs/load-test-c3c5c6.md）をC5向けに
 *  複製・調整したもの（実施46）。S3のスループット試験
 *  （docs/s3-throughput-findings-for-c6.md）が露見させた2つのOSAシム
 *  潜在欠陥（(A)キューのメッセージ毎malloc・ISR内malloc，(B)TX
 *  バッファの毎パケット動的malloc）をC5で検証するための専用アプリ．
 *  apps/tcp_socket_echo・apps/udp_socket_echoのボイラープレートを
 *  そのまま流用し（両アプリは他エージェントが使用中の可能性がある
 *  ため直接改造せず新規ディレクトリとして複製），
 *  ・TCP echo（port 8）
 *  ・UDP echo（port 9）
 ...
 *  ようにしている（C3実施1と同じ判断基準を踏襲）。
 */
#ifndef LOAD_TEST_C5_H
#define LOAD_TEST_C5_H
 ...
 *  8/9を再利用する（単体flash運用．C3実施1と同一番号）。
 */
