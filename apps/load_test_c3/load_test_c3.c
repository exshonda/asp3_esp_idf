/*
 *  C3負荷試験デモ（ASP3＋esp_wifi blob＋os_adapter shim＋lwIP）
 *
...
main_task(EXINF exinf)
{
	...
	syslog(LOG_NOTICE,
		   "load_test_c3: LOAD_TEST_STATIC_TXBUF=ON "
		   "(tx_buf_type=0, static_tx_buf_num=16)");
#else
	syslog(LOG_NOTICE,
		   "load_test_c3: LOAD_TEST_STATIC_TXBUF=OFF "
		   "(default dynamic tx_buf)");
#endif

	syslog(LOG_NOTICE, "load_test_c3: esp_wifi_init");
	...
	syslog(LOG_NOTICE, "load_test_c3: SSID='%s'", WIFI_SSID);
	...
	syslog(LOG_NOTICE, "load_test_c3: esp_wifi_connect -> %d", (int_t)err);
	...
	for (retry = 0; retry < 20; retry++) {
		...
		if (conn_state == 1) {
			syslog(LOG_NOTICE, "load_test_c3: CONNECTED, waiting for DHCP");
			break;
		}
		if (conn_state == -1) {
			syslog(LOG_NOTICE, "load_test_c3: retry connect (%d)", retry);
			conn_state = 0;
	...
	if (conn_state != 1) {
		syslog(LOG_NOTICE, "load_test_c3: FAILED (timeout)");
		return;
	}
	...
	for (retry = 0; retry < 20; retry++) {
		...
		if (ip != 0) {
			syslog(LOG_NOTICE, "load_test_c3: IP acquired: %d.%d.%d.%d",
	...
	if (ip == 0) {
		syslog(LOG_NOTICE, "load_test_c3: DHCP FAILED (timeout)");
	}
	syslog(LOG_NOTICE, "load_test_c3: main_task done "
...
tcp_echo_session(int conn_sock)
{
	...
	for (;;) {
		...
		if (wlen != rlen) {
			syslog(LOG_NOTICE, "load_test_c3: tcp send() error (wlen=%d)",
				   (int_t) wlen);
			g_tcp_send_errors++;
			break;
		}
	...
	syslog(LOG_NOTICE, "load_test_c3: tcp client disconnected "
...
tcp_echo_task(EXINF exinf)
{
	...
	syslog(LOG_NOTICE,
		   "load_test_c3: starting TCP echo server (port %d)",
		   (int_t) TCP_ECHO_PORT);

	listen_sock = socket(AF_INET, SOCK_STREAM, 0);
	if (listen_sock < 0) {
		syslog(LOG_ERROR, "load_test_c3: tcp socket() failed (%d)",
			   (int_t) listen_sock);
		return;
	}
	...
	if (bind(listen_sock, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		syslog(LOG_ERROR, "load_test_c3: tcp bind() failed");
		(void) close(listen_sock);
		return;
	}

	if (listen(listen_sock, 1) < 0) {
		syslog(LOG_ERROR, "load_test_c3: tcp listen() failed");
		(void) close(listen_sock);
		return;
	}

	syslog(LOG_NOTICE, "load_test_c3: tcp listening on 0.0.0.0:%d",
		   (int_t) TCP_ECHO_PORT);
	...
	for (;;) {
		...
		if (conn_sock < 0) {
			syslog(LOG_NOTICE, "load_test_c3: tcp accept() failed");
			continue;
		}
		...
		g_tcp_sessions++;
		syslog(LOG_NOTICE,
			   "load_test_c3: tcp client connected %d.%d.%d.%d:%d",
...
udp_echo_task(EXINF exinf)
{
	...
	syslog(LOG_NOTICE,
		   "load_test_c3: starting UDP echo server (port %d)",
		   (int_t) UDP_ECHO_PORT);

	sock = socket(AF_INET, SOCK_DGRAM, 0);
	if (sock < 0) {
		syslog(LOG_ERROR, "load_test_c3: udp socket() failed (%d)",
			   (int_t) sock);
		return;
	}
	...
	if (bind(sock, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
		syslog(LOG_ERROR, "load_test_c3: udp bind() failed");
		(void) close(sock);
		return;
	}

	syslog(LOG_NOTICE, "load_test_c3: udp listening on 0.0.0.0:%d (UDP)",
		   (int_t) UDP_ECHO_PORT);
	...
	for (;;) {
		...
		if (rlen < 0) {
			syslog(LOG_NOTICE, "load_test_c3: udp recvfrom() failed");
			continue;
		}
		g_udp_datagrams++;
		...
		wlen = sendto(sock, buf, (size_t) rlen, 0,
					  (struct sockaddr *) &peer, peerlen);
		if (wlen != rlen) {
			syslog(LOG_NOTICE, "load_test_c3: udp sendto() error (wlen=%d)",
				   (int_t) wlen);
			g_udp_send_errors++;
		} else {
...
mon_task(EXINF exinf)
{
	...
	for (;;) {
		 ...
		 */
		syslog(LOG_NOTICE,
			   "load_test_c3: MON uptime=%us heap_free=%u "
