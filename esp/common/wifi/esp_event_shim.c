 ...
 *  ＝ここを揃えないと，esp_event.h を見る側（esp_wifi_adapter.c）の
 *  宣言と本定義でプロトタイプが食い違う（表現は同じためリンクは
 *  通ってしまうので，コンパイル時に検出できる形にしておく）．
 *  hal供給（C3/C6の既存ビルド）は従来どおり `void *`．
 */
int
esp_event_post(const char *event_base, int32_t event_id,
