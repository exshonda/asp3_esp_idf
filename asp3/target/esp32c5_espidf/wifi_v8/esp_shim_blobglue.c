 ...
 *
 *  ★hal(v8)統一（docs/c5-hal-v8-unification-memo.md）でv9側の実施45
 *  （docs/c5-bringup.md）の修正をそのまま移植した：
 *
 *  (1) g_misc_nvsのC定義「void *g_misc_nvs = NULL;」はリンクされない．
 *      esp32c5.rom.net80211.ld（v8/v9共通．同一ROM＝esp_wifi_v8.cmakeも
 *      同じROM ldをリンクする）が`g_misc_nvs = 0x4085ff7c;`（ROM
 *      インターフェースデータ領域）を絶対シンボルとして供給し，リンカ
 *      スクリプト定義がオブジェクト定義より優先されるため，この
 *      セルの初期化は実行時の代入でしか行えない．
 *  (2) net80211 blob（v8のlibnet80211.a／libcore.aも`nm`実測でg_misc_nvsを
 *      未定義参照として要求することを確認済み＝v9と同じ経路）は
 *      NULLチェック無しで+4/+8（WPSステータス語）をlwする箇所が
 *      あるため，NULL固定のままだとLoad access fault（アドレス4読み）
 *      でクラッシュしうる．
 *
 *  対処（v9版と同一）：blobが esp_wifi_init 内で必ず呼ぶ misc_nvs_init()
 *  で，静的なゼロ初期化済み構造体を指させる．全参照サイトは+4/+8しか
 *  読まないため，全ゼロ＝「WPS無効・NVS無し」として安全に振る舞う．
 */
