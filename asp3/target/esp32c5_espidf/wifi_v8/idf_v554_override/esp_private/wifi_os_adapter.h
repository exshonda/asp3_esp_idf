...
/*
 *  v5.5.4統一（docs/blob-unify-v554.md）：本ファイルはESP-IDF v5.5.4
 *  （`~/tools/esp-idf/components/esp_wifi/include/esp_private/
 *  wifi_os_adapter.h`）からの verbatim コピー（Espressif Apache-2.0
 *  ヘッダ含め無改変）。hal/asp3_core禁則には抵触しない（target側の
 *  新規ファイル．hal submoduleの当該ファイルは編集していない）。
 *
 *  存在理由：hal（esp-hal-3rdparty submodule）のwifi_os_adapter.hは
 *  `_wifi_disable_ac_ax`フィールド（`#if CONFIG_SOC_WIFI_HE_SUPPORT`
 *  ガード）を欠く古い版であり，C5/C6（soc_caps.hでSOC_WIFI_HE_SUPPORT=1）
 *  向けにv5.5.4 blob（net80211.a/pp.a）をリンクするとwifi_osi_funcs_t
 *  構造体のサイズ・`_magic`フィールドのオフセットがずれ，blob内部の
 *  整合性チェック（esp_wifi_internal_osi_funcs_md5_check）が失敗する
 *  （C3はSOC_WIFI_HE_SUPPORT非該当のためこの問題は起きず，本override
 *  は不要＝esp_wifi.cmake(c3)は素のhalヘッダのまま）。
 *
 *  esp_wifi.cmake（本ディレクトリのesp_wifi_v8.cmake）がこのディレクトリ
 *  （idf_v554_override/）をhalのesp_wifi/includeより«前»のインクルード
 *  パスに追加し，このファイル1つだけをシャドウする（他のヘッダ名は
 *  このディレクトリに存在しないため素通りしてhal側にフォールバックする
 *  ＝サロゲート最小差し替え）。esp_wifi_adapter.cが`._wifi_disable_ac_ax`
 *  フィールドを実際に埋める（CONFIG_SOC_WIFI_HE_SUPPORT=1はC5/C6とも
 *  既存のsdkconfig stub／hal nuttx sdkconfig.h経由で既に定義済み）。
 */
