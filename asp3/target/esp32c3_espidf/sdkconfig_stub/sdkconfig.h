 ...
 *        `nuttx/esp32c3/include/sdkconfig.h` からの **verbatim コピー**
 ...
 *  C3 ターゲットは従来これを直接 include していた（`${ESP_HAL_DIR}/
 *  nuttx/esp32c3/include`）。供給を esp-idf submodule へ移す（＝hal 参照
 ...
 *
 *  ★**CONFIG_* は 1 ビットも変えていない**（C6 sdkconfig_stub と同じ方針＝
 *  供給移行で「動いていた設定」を同時に動かさない）。差分は本コメントのみ。
 *
 *  ★同じ hal ディレクトリには irq.h も同梱されるが，**本ビルドは
 *  sdkconfig.h しか消費していない**（実測：baseline ビルドの
 *  `ninja -t deps | grep nuttx/esp32c3` が sdkconfig.h の 1 本のみ）＝
 *  vendor はこの 1 ファイルで足りる。
 *  ------------------------------------------------------------------
 */
