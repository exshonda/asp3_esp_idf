...
/*
 *		ESP32-C3 USB Serial/JTAGコントローラ用 簡易SIOドライバ
 *		（esp-hal LL層版・非TECS版専用）
 *
 *  asp3_core の arch/riscv_gcc/esp32c3/esp32c3_usbjtag.c（レジスタ直
 *  叩き版）を，esp-hal-3rdparty の LL 層（hal/usb_serial_jtag_ll.h＝
 *  static inline のレジスタ薄層・RTOS非依存）で置き換えたもの
 *  （Phase B-1：esp-hal統合の実証）．公開シンボル（esp32c3_usbjtag_*）
 ...
 */
void
esp32c3_usbjtag_initialize(void)
{
	SIOPCB	*p_siopcb;
	uint_t	i;
 ...
 */
void
esp32c3_usbjtag_terminate(void)
{
	uint_t	i;

	for (i = 0; i < TNUM_SIOP; i++) {
		esp32c3_usbjtag_cls_por(&(siopcb_table[i]));
	}
}
 ...
 */
SIOPCB *
esp32c3_usbjtag_opn_por(ID siopid, EXINF exinf)
{
	SIOPCB	*p_siopcb;
 ...
 */
void
esp32c3_usbjtag_cls_por(SIOPCB *p_siopcb)
{
	uint32_t	retry;
 ...
 */
bool_t
esp32c3_usbjtag_snd_chr(SIOPCB *p_siopcb, char c)
{
	if (usb_serial_jtag_ll_txfifo_writable() != 0) {
 ...
 */
int_t
esp32c3_usbjtag_rcv_chr(SIOPCB *p_siopcb)
{
	uint8_t	c;
 ...
 */
void
esp32c3_usbjtag_ena_cbr(SIOPCB *p_siopcb, uint_t cbrtn)
{
	switch (cbrtn) {
	case SIO_RDY_SND:
 ...
 */
void
esp32c3_usbjtag_dis_cbr(SIOPCB *p_siopcb, uint_t cbrtn)
{
	switch (cbrtn) {
	case SIO_RDY_SND:
		usb_serial_jtag_ll_disable_intr_mask(
 ...
 */
static void
esp32c3_usbjtag_isr_siop(SIOPCB *p_siopcb)
{
	uint32_t	stat;
	...
	if ((stat & USB_SERIAL_JTAG_INTR_SERIAL_IN_EMPTY) != 0U) {
		...
		usb_serial_jtag_ll_clr_intsts_mask(
					USB_SERIAL_JTAG_INTR_SERIAL_IN_EMPTY);
		esp32c3_usbjtag_irdy_snd(p_siopcb->exinf);
	}
	if ((stat & USB_SERIAL_JTAG_INTR_SERIAL_OUT_RECV_PKT) != 0U) {
		...
		while (usb_serial_jtag_ll_rxfifo_data_available() != 0) {
			esp32c3_usbjtag_irdy_rcv(p_siopcb->exinf);
		}
	}
}
 ...
 */
void
esp32c3_usbjtag_isr(ID siopid)
{
	esp32c3_usbjtag_isr_siop(get_siopcb(siopid));
}
