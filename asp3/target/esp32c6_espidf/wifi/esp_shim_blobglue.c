 ...
 *
 *  ★実施90（docs/wifi-shim-c6.md）で実施45（docs/c5-bringup.md）を
 *  移植：旧コメントの推定「blobはNULLチェック後に使う」は誤りだった．
 *  C6のlibnet80211.a（wl_cnx.o等）を逆アセンブルすると，
 *  esp_wifi_get_wps_status_internal他，C5実施45が確認した9箇所と
 *  同一パターンで`lw a5,g_misc_nvs`→`lw a0,8(a5)`（NULLチェック
 *  無しで+4/+8をlw）していることを確認済み（C5と同一blob世代）．
 *  g_misc_nvs=NULLだとアドレス4/8のロードとなり，バスがトラップする
 *  チップ（C5で実測確認）ではLoad access faultになる．
 *
 *  C5との相違点（要注意）：C5はROM ld（`esp32c5.rom.net80211.ld`）が
 *  `g_misc_nvs = 0x4085ff7c;`という絶対シンボルを供給し，C定義を
 *  リンクから排除していた（実行時代入が必須の理由）．
 *  **C6のROM ld一式（esp32c6.rom.*.ld）にはg_misc_nvsの絶対シンボル
 *  定義が存在しない**（`grep -rl g_misc_nvs hal/components/esp_rom/
 *  esp32c6/ld/`で確認・0件）——実施45申し送り事項1の「C6 ROM ldに
 *  同シンボルあり」という記述は未検証の推測であり，本ラウンドの
 *  実測で誤りと判明した．C6では本ファイルのC定義がそのまま
 *  リンクされる．
 *
 *  ただし対処自体はC5と同じにする：静的初期化子だけに頼らず，
 *  blobが esp_wifi_init 内で必ず呼ぶ misc_nvs_init() で明示的に
 *  ゼロ構造体を指させる（C定義の初期値に依存する経路を減らし，
 *  C5と対処ロジックを揃えることで将来のリンク方式変更にも頑健にする）．
