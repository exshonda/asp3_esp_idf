 ...
 *
 *  存在理由：C5版（asp3/target/esp32c5_espidf/wifi_v8/
 *  idf_v554_override/）と同一の事象・同一の対処。hal（esp-hal-3rdparty
 *  submodule）のwifi_os_adapter.hは`_wifi_disable_ac_ax`フィールド
 *  （`#if CONFIG_SOC_WIFI_HE_SUPPORT`ガード）を欠く古い版であり，
 *  C6（soc_caps.hでSOC_WIFI_HE_SUPPORT=1，CONFIG_SOC_WIFI_HE_SUPPORTは
 *  hal/nuttx/esp32c6/include/sdkconfig.hで既に1）向けにv5.5.4 blob
 *  （net80211.a/pp.a）をリンクするとwifi_osi_funcs_t構造体のサイズ・
 *  `_magic`フィールドのオフセットがずれ，blob内部の整合性チェック
 *  （esp_wifi_internal_osi_funcs_md5_check）が失敗する。
 *
 *  esp_wifi.cmake（本ディレクトリ）がこのディレクトリ
